...
Apresentado em [visão geral do serviço de registro em log centralizado no Lync Server 2013](lync-server-2013-overview-of-the-centralized-logging-service.md), os principais elementos da definição de um provedor para uso em um cenário são:
  ...
  - **Níveis**     de log OCSLogger forneceu a opção para escolher vários níveis de detalhes para os dados coletados. Este recurso é parte integrante do serviço de registro em log centralizado e de cenários e é definido pelo parâmetro **Type** . Você pode escolher entre estas opções:
    
      - **Todos os**     Coleta mensagens de rastreamento do tipo fatal, erro, aviso e informações no log do provedor definido.
      ...
      - **Informações**     Coleta somente as mensagens de rastreamento que indicam uma mensagem informativa do provedor definido, além de mensagens fatais, de erro e de aviso.
    
      - **Detalhado**     Coleta todas as mensagens de rastreamento do tipo fatal, erro, aviso e informações do provedor definido.

  - **Sinalizadores**     OCSLogger forneceu a opção para escolher sinalizadores para cada provedor que definia o tipo de informação que você pode recuperar dos arquivos de rastreamento. Você pode escolher os seguintes sinalizadores, com base no provedor:
    
      - **TF \_ A conexão**     fornece entradas de log relacionadas à conexão. Esses logs incluem informações sobre as conexões estabelecidas com e a partir de um componente específico. Isso também pode incluir informações significativas no nível da rede (ou seja, para componentes sem o conceito de uma conexão).
    
      - **TF \_ A segurança**     fornece todos os eventos/entradas de log relacionados à segurança. Por exemplo, para SipStack, são eventos de segurança como falha na validação do domínio e falhas de autenticação/autorização do cliente.
    
      - **TF \_ O diag**     fornece eventos de diagnóstico que você pode usar para diagnosticar ou solucionar problemas do componente. Por exemplo, para SipStack, são falhas de certificado e avisos/erros de DNS.
    
      - **TF \_ Protocol**     fornece mensagens de protocolo como mensagens de SIP e pacote de codecs de comunidade combinados.
    
      - **TF \_ O componente**     permite o registro em log dos componentes especificados como parte dos provedores.
    
      - **Todos os**     Define todos os sinalizadores disponíveis para o provedor.
...
–Level coleta mensagens fatais, de erro, aviso e informações. Os sinalizadores usados são todos aqueles definidos para o provedor Lyss e incluem a conexão de TF \_ , TF \_ diag and TF \_ Protocol.
