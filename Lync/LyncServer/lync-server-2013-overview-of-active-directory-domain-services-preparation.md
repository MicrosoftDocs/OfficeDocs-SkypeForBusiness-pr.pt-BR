...
No Lync Server 2013, você delega a instalação e administração, concedendo permissões de configuração ao grupo universal RTCUniversalServerAdmins para que os membros desse grupo possam instalar e ativar o Lync Server 2013 em um servidor local (depois que o servidor tiver sido adicionado ao topologia, publicada e habilitada). Os usuários delegados devem ser administradores locais no computador onde estão instalando e ativando o Lync Server 2013, mas não precisam ser membros do grupo de administradores de domínio. Você também pode conceder permissões para objetos em unidades organizacionais (UOs) especificadas, de forma que os membros dos grupos universais criados durante a preparação da floresta possam acessar esses objetos sem ser membros do grupo Administradores de domínio.
