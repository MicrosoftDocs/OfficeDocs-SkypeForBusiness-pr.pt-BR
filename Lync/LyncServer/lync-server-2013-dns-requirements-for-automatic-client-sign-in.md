...
Para habilitar a configuração automática para seus clientes, você deve criar um registro SRV de DNS interno que mapeia um dos seguintes registros para o nome de domínio totalmente qualificado (FQDN) do pool de front-ends ou servidor Standard Edition que distribui solicitações de entrada de clientes Lync:

  - \_sipinternaltls. \_ TCP.\<domain\> -para conexões TLS internas
