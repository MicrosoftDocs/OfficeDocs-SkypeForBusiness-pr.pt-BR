---
title: 'Fase 8: mover da implantação piloto para a produção'
ms.reviewer: ''
ms.author: kenwith
author: kenwith
f1.keywords:
- NOCSH
TOCTitle: 'Phase 8: Move from pilot deployment into production'
ms:assetid: 00b22ecf-0e28-4bde-8f75-2e82c5e2e718
ms:mtpsurl: https://technet.microsoft.com/en-us/library/JJ687954(v=OCS.15)
ms:contentKeyID: 49733539
ms.date: 07/23/2014
manager: serdars
mtps_version: v=OCS.15
ms.openlocfilehash: 07ab5e017f165a3dc5d9883f14b3711501a0e0d4
ms.sourcegitcommit: 88a16c09dd91229e1a8c156445eb3c360c942978
ms.translationtype: MT
ms.contentlocale: pt-BR
ms.lasthandoff: 02/15/2020
ms.locfileid: "42034131"
---
<div data-xmlns="http://www.w3.org/1999/xhtml">

<div class="topic" data-xmlns="http://www.w3.org/1999/xhtml" data-msxsl="urn:schemas-microsoft-com:xslt" data-cs="http://msdn.microsoft.com/">

<div data-asp="http://msdn2.microsoft.com/asp">

# <a name="phase-8-move-from-pilot-deployment-into-production"></a>Fase 8: mover da implantação piloto para a produção

</div>

<div id="mainSection">

<div id="mainBody">

<span> </span>

_**Última modificação do tópico:** 2012-10-15_

Os tópicos desta seção descrevem tarefas que você deve concluir antes de mover sua implantação do Lync Server 2013 de uma implantação piloto para uma implantação no nível de produção.

<div>

## <a name="in-this-section"></a>Nesta seção

  - [Configure o tráfego de mídia e rotas de federação](configure-federation-routes-and-media-traffic_1.md)

  - [Mover os usuários restantes para o Lync Server 2013](move-remaining-users-to-lync-server-2013_1.md)

  - [Configurar o Gateway XMPP no Lync Server 2013](configure-xmpp-gateway-on-lync-server-2013_1.md)

</div>

</div>

<span> </span>

</div>

</div>

</div>

