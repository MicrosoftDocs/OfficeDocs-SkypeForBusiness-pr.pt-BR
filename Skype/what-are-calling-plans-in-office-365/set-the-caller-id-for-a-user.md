...
O sistema telefônico no Office 365 fornece uma ID de chamador padrão que é o número de telefone atribuído do usuário. Você pode alterar ou bloquear a ID de chamador (também chamada de uma ID de linha chamar) para um usuário. Saiba mais sobre como usar o ID do chamador em sua organização indo [como ID do chamador pode ser usado na sua organização](how-can-caller-id-be-used-in-your-organization.md).
...
- **Bloquear seu ID de chamadas de saída** Você pode bloquear a ID de chamadas de saída sejam enviadas em chamadas PSTN de saída de um usuário. Esse procedimento bloqueará o seu número de telefone seja exibido no telefone de uma pessoa que está sendo chamado.
