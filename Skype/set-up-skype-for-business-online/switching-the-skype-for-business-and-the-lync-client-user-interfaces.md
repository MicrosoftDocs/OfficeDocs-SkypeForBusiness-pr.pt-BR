...
Para Skype para organizações Business Online, você pode usar o PowerShell remoto no Office 365 para habilitar sua Skype para usuários comerciais para usar o Skype para o cliente de negócios ou o Skype para interface de usuário do cliente de negócios (Lync). A configuração padrão é para os usuários utilizem o Skype para interface de usuário do cliente de negócios. Se você preferir usar a experiência do cliente Lync, é possível gerenciar o comportamento do cliente primeiro lançamento para exibir a interface de usuário do Lync seguindo as etapas neste tópico.
...
```Grant-CsClientPolicy-PolicyName ClientPolicyDisableSkypeUI -Identity <username>```|O usuário será solicitado para alternar para o Skype para interface de usuário do cliente de negócios (Lync). Um administrador pode alterar a configuração no futuro que alternará para o Skype para interface de usuário do cliente de negócios. |
...
|A política não está definida.  <br/> |Os usuários verão o Skype para interface de usuário do cliente de negócios (Lync) nunca se a diretiva não estiver definida. Sempre usarão o Skype para interface de usuário do cliente de negócios.  <br/> |O usuário continuará a utilizar a interface do cliente Skype for Business.  <br/> |
