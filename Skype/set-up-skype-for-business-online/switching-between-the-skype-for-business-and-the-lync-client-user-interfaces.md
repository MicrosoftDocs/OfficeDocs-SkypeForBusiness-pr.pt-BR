...
# Alternando entre as interfaces do usuário do cliente Skype for Business e do cliente Lync

> [!IMPORTANT]
> Este artigo foi traduzido por um sistema de tradução automática, leia o [aviso de isenção de responsabilidade](a2394a4c-7522-484c-a047-7b3289742be0.md#MT_Footer). Para sua referência, veja a versão em inglês deste artigo [aqui](https://support.office.com/en-us/article/a2394a4c-7522-484c-a047-7b3289742be0). 
  
Para as organizações que têm o Skype for Business Online, você pode usar o PowerShell Remoto no Office 365 para permitir que os usuários do Skype for Business utilizem a interface do usuário do cliente Skype for Business ou do cliente Skype for Business (Lync). Na configuração padrão, os usuários devem utilizar a interface do cliente Skype for Business. Se você preferir usar a experiência de cliente do Lync, pode gerenciar o comportamento do cliente na primeira inicialização para exibir a interface de usuário do Lync seguindo as etapas mostradas posteriormente neste tópico.
  
> [!NOTE]
> A experiência do cliente Lync 2013 não é uma opção para as versões do cliente Skype for Business 2016. Antes de tentar configurar seu ambiente para usar o cliente Lync 2013, verifique a versão do cliente para garantir que ela não começa com o número 16. Por exemplo: 16.x.x.x. 
  
> [!TIP]
> Se você deseja alternar facilmente a interface do usuário e não quer as etapas manuais, consulte o [Centro de Download Microsoft ](https://go.microsoft.com/fwlink/?LinkId=532431) obter um script PowerShell para tornar mais fácil.
  
## Alternando a interface do Skype for Business para os usuários

O módulo do Windows PowerShell para Skype for Business Online permite que você crie uma sessão remota do Windows PowerShell que se conecta ao Skype for Business Online. Neste módulo, que é suportado apenas em computadores de 64 bits pode ser baixado do Microsoft Download Center no [Módulo do Windows PowerShell para Skype for Business Online](https://go.microsoft.com/fwlink/?LinkId=294688). Para outras informações, consulte [Configurar o computador para Skype for Business Online gerenciamento](https://go.microsoft.com/fwlink/?LinkId=534539).
  
> [!IMPORTANT]
> A configuração da política  _Global_ para alternar a interface do usuário não será aplicada aos usuários que já têm uma política personalizada aplicada. Para alterar a interface, você precisará executar o seguinte para cada usuário que tiver uma política personalizada aplicada:
...
> [!CAUTION]
> A política  _ClientPolicyEnableSkypeUI_ substituirá a configuração de política existente do usuário.
...
![Skype for Business Online - Habilitar interface do usuário](../images/596aef69-41dc-4e1e-b689-2b7009ae58a1.gif)
...
![Skype for Business Online - Interface do Usuário Desabilitada](../images/61c645e0-67fc-4e03-803c-b7028a47dae3.gif)
...
> [!NOTE]
>  O nome do usuário é o nome da conta do usuário à qual a política deve ser atribuída. O nome da conta do usuário pode ser inserido em um dos seguintes formatos:>  Endereço SIP do usuário>  Nome UPN do usuário>  Domínio\\nome de usuário do usuário>  Nome para exibição do Active Directory do usuário
  
[Usando o Windows PowerShell para gerenciar o Lync Online](https://go.microsoft.com/fwlink/?LinkID=525453)
  
## Configurações de política do Skype for Business Online
...
|:-----|:-----|
|A política não está definida.  <br/> |O usuário continuará a utilizar a interface do cliente Skype for Business.  <br/> |
|
```
Grant-CsClientPolicy -PolicyName ClientPolicyEnableSkypeUI
```

|O usuário continuará a utilizar a interface do cliente Skype for Business.  <br/> |
|
```
Grant-CsClientPolicy -PolicyName ClientPolicyDisableSkypeUI
```

|O usuário será solicitado a alternar para a interface do cliente Skype for Business (Lync). Ele poderá alternar posteriormente.  <br/> |
|
```
Grant-CsClientPolicy -PolicyName ClientPolicyEnableSkypeUI -Identity <username>
```

|O usuário utilizará a interface do cliente Skype for Business.  <br/> |
|
```
Grant-CsClientPolicy-PolicyName ClientPolicyDisableSkypeUI -Identity <username>
```

|O usuário será solicitado a alternar para a interface do cliente Skype for Business (Lync). No futuro, o administrador poderá alterar a configuração de modo que o usuário alterne para a interface do cliente Skype for Business.  <br/> |
...
|O usuário será solicitado a alternar para a interface do cliente Skype for Business.  <br/> |O usuário continuará a utilizar a interface do cliente Skype for Business.  <br/> |
|
```
Grant-CsClientPolicy -PolicyName ClientPolicyDisableSkypeUI
```

|O usuário continuará a utilizar a interface do cliente Skype for Business (Lync).  <br/> |O usuário será solicitado a alternar para a interface do cliente Skype for Business (Lync).  <br/> |
|A política não está definida.  <br/> |Os usuários nunca verão a interface do cliente Skype for Business (Lync) se a política não estiver definida. Eles sempre utilizarão a interface do cliente Skype for Business.  <br/> |O usuário continuará a utilizar a interface do cliente Skype for Business.  <br/> |
...
Para começar a usar o Windows PowerShell, consulte estes tópicos:
  
- [Seis razões por que talvez você queira usar o Windows PowerShell para gerenciar o Office 365]( https://go.microsoft.com/fwlink/?LinkId=525041)
    
- [Melhores maneiras de gerenciar o Office 365 com o Windows PowerShell]( https://go.microsoft.com/fwlink/?LinkId=525142)
    
## Comportamentos do cliente na primeira inicialização

Por padrão, quando os usuários iniciam o Skype for Business pela primeira vez, eles sempre veem a interface do usuário do Skype for Business -- mesmo tendo selecionado a experiência de cliente do Lync configurando a política de cliente para essa experiência ( `Grant-CsClientPolicy -PolicyName ClientPolicyDisableSkypeUI`) conforme descrito anteriormente. Após alguns minutos, os usuários serão solicitados a mudar para o modo Lync.
...
### Controlar a exibição do tutorial da tela de boas-vindas
...
### Desativar o tutorial do cliente
...
## Criar um Objeto de Política de Grupo para modificar o Registro em um computador que ingressou no domínio
 ...
 **Usar o GPO para atribuir política**
