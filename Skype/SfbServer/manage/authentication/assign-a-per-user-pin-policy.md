...
description: 'Resumo: atribua políticas de PIN por usuário aos usuários no Skype for Business Server.'
...
A política de PIN (número de identificação pessoal) da conferência discada é uma das configurações individuais de uma conta de usuário que pode ser configurada no Skype for Business Server Painel de Controle.
...
7. Em **Atribuir Políticas**, em **Política de PIN**, execute uma das seguintes ações:
    ...
    > [!NOTE]
    > Como há várias políticas que você pode configurar usando a caixa de  diálogo Atribuir Políticas, **\<Keep as is\>** é selecionada por padrão para cada política na caixa de diálogo. Para continuar usando a política previamente atribuída ao usuário, basta não fazer nenhuma alteração nessa configuração.

   - Permita Skype for Business Server escolha automaticamente a política de nível global ou, se definida, a política de nível de site.
...
## <a name="assigning-a-per-user-pin-policy-by-using-windows-powershell-cmdlets"></a>Atribuir uma política Per-User PIN usando cmdlets Windows PowerShell dados

Você pode atribuir políticas de PIN por usuário usando Windows PowerShell e o cmdlet **Grant-CsPinPolicy**. Você pode executar este cmdlet usando o Shell de gerenciamento do Skype for Business Server ou uma sessão remota do Windows PowerShell. Para obter detalhes sobre como usar Windows PowerShell para se conectar ao Skype for Business Server, consulte Administração remota do [PowerShell do Microsoft Lync"](https://blog.insideo365.com/2011/08/remote-lync-powershell-administration/). O processo é o mesmo no Skype for Business Server.
...
- O seguinte comando atribui a política PIN por usuário RedmondUsersPinPolicy para todos os usuários que trabalham na cidade de Redmond. Para obter detalhes sobre o parâmetro LdapFilter usado neste comando, consulte [Get-CsUser](/powershell/module/skype/get-csuser).
...
Para obter detalhes, [consulte Grant-CsPinPolicy](/powershell/module/skype/grant-cspinpolicy).
