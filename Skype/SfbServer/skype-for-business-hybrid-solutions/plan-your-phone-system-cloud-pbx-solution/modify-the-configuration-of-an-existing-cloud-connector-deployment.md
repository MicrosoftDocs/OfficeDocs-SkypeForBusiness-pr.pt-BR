...
description: Siga as etapas deste tópico para modificar a configuração de uma implantação Skype for Business Cloud Connector Edition 1.4.1 ou posterior.
ms.openlocfilehash: 5d0771e1f6a62015cf040a899c88696016366e47
ms.sourcegitcommit: 556fffc96729150efcc04cd5d6069c402012421e
ms.translationtype: MT
ms.contentlocale: pt-BR
ms.lasthandoff: 08/26/2021
ms.locfileid: "58590005"
---
...
Siga as etapas deste tópico para modificar a configuração de uma implantação Skype for Business Cloud Connector Edition 1.4.1 ou posterior. 
  
## <a name="modify-the-configuration-of-a-single-site"></a>Modificar a configuração de um único site
<a name="BKMK_SIngleSite"> </a>

Se houver apenas um dispositivo no site, quando quiser alterar as configurações após a implantação do dispositivo, você poderá modificar o arquivo CloudConnector.ini e iniciar a implantação novamente.
...
2. Execute o seguinte cmdlet para não fazer o registro do dispositivo:
    
   ```powershell
   Unregister-CcAppliance
   ```

3. Atualize o CloudConnector.ini no Diretório do Dispositivo.
...
6. Execute o seguinte cmdlet para instalar Skype for Business Cloud Connector Edition:
...
## <a name="modify-the-configuration-of-your-microsoft-365-or-office-365-organization-to-enable-automatic-updates"></a>Modificar a configuração da sua Microsoft 365 ou Office 365 para habilitar atualizações automáticas
<a name="BKMK_MultipleSites"> </a>

Para habilitar atualizações automáticas do sistema operacional e atualizações automáticas bits, você deve usar a conta de administrador Skype for Business locatário para gerenciamento online e usar o PowerShell remoto de locatário da seguinte forma.
  
Se você desabilitou as atualizações automáticas do sistema operacional ou as atualizações automáticas bits, seu host e máquina virtual poderão perder atualizações importantes Windows e o Cloud Connector não atualizará para a nova versão automaticamente. É altamente recomendável habilitar atualizações automáticas.
...
2. Crie uma janela de tempo de atualização automática para o locatário.
   ...
   - Para uma janela de horário diária, apenas a hora de início e a duração são necessárias. 
    
   - Para uma janela de tempo semanal, dias da semana são necessários, o que pode ser um único dia ou vários dias.
   ...
   - Cada locatário pode ter janelas de 20 horas definidas. A janela de tempo padrão será criada para um novo locatário como a janela de tempo padrão para atualização do sistema operacional e atualização bits. Execute os seguintes cmdlets para definir a janela diária, semanal ou mensal:
   ...
   - Atribua janelas de tempo de atualização ao site. 
     ...
     O tempo de atualização bits e as janelas de tempo de atualização do sistema operacional são configurados separadamente. Ambos podem ser atribuídos a janelas única ou múltipla. Cada janela de tempo pode ser atribuída a diferentes sites e finalidades diferentes (atualização de bits e atualização do sistema operacional). Execute o seguinte cmdlet para definir a janela de tempo do site: 
...
As alterações administrativas na Microsoft 365 ou Office 365 para o Cloud Connector são feitas de uma conta com as permissões necessárias. Em versões do Cloud Connector antes do 2.0, essa conta é uma conta de administrador de locatário global dedicada. Nas versões 2.0 e posteriores do Cloud Connector, essa conta pode ser uma conta Microsoft 365 ou Office 365 com direitos Skype for Business Administrador.
  
Se as credenciais da sua conta de administrador mudarem no Microsoft 365 ou no Office 365, você também precisará atualizar as credenciais armazenadas em cache localmente no Cloud Connector executando o seguinte comando administrador do PowerShell em cada dispositivo do Cloud Connector implantado:
...
5. Iniciar um console do PowerShell como administrador e executar "Register-CcAppliance -Local" para inserir as senhas após a descrição. Insira a mesma senha inserida antes para a implantação do Cloud Connector.
    
Por padrão, VmAdmin e DomainAdmin usam a mesma senha que CceService. Se as senhas DomainAdmin, VMAdmin e CceService retornadas na etapa 1 são diferentes, você deve executar as seguintes etapas:
...
2. Quando solicitado a nova credencial da conta, insira a senha da senha DomainAdmin retornada na etapa 1.
    
2. Execute Set-CcCredential -AccountType VmAdmin da seguinte forma:
    
1. Quando solicitado a usar a credencial da conta antiga, insira a credencial usada para a senha do CceService.
    
2. Quando solicitado a nova credencial da conta, insira a senha da senha VmAdmin retornada na etapa 1. 
...
- Por padrão, a conta CceService é configurada como "Senha nunca expira". Quando você atualiza a senha, a Microsoft recomenda manter essa configuração.
    
- Você deve atualizar a senha durante períodos de uso que não são de pico e fora das janelas de tempo de atualização automática para bits ou Windows atualizações. Quando você atualiza a senha, o dispositivo precisa ser esvaziado e reiniciado, o que leva algum tempo. Reiniciar o dispositivo interromperá as operações de atualização automática. 
    
- Quando você alterar a senha da conta CceService, precisará especificar todas as credenciais e atualizá-las no arquivo armazenado localmente. 
...
5. Execute o Restore-CcCredentials cmdlet para inserir as senhas após a descrição. 
    
    Insira a mesma senha inserida antes para a implantação do Cloud Connector, exceto para a conta CceService. Para a conta CceService, insira sua nova senha. Certifique-se de que a nova senha da conta CceService seja a mesma para todos os dispositivos no site PSTN.
    
6. Por padrão, VmAdmin e DomainAdmin usam a mesma senha que CceService. Se as senhas DomainAdmin, VMAdmin e CceService retornadas na etapa 1 são diferentes, você deve executar as seguintes etapas:
    
7. Execute Set-CcCredential -AccountType DomainAdmin da seguinte forma:
    
   - Quando solicitado a usar a credencial da conta antiga, insira a credencial usada para a senha do CceService.
    
   - Quando solicitado a nova credencial da conta, insira a senha da senha DomainAdmin retornada na etapa 1.
    
8. Execute Set-CcCredential -AccountType VmAdmin da seguinte forma:
    
   - Quando solicitado a usar a credencial da conta antiga, insira a credencial usada para a senha do CceService.
    
   - Quando solicitado a nova credencial da conta, insira a senha da senha VmAdmin retornada na etapa 1. 
...
Para adicionar um novo domínio SIP (ou vários domínios SIP) à implantação existente do Cloud Connector, faça o seguinte:
  
1. Certifique-se de ter concluído as etapas para atualizar seu domínio em Microsoft 365 ou Office 365 e ter a capacidade de adicionar registros DNS. Para obter mais informações sobre como configurar seu domínio em Microsoft 365 ou Office 365, consulte Adicionar um domínio a Microsoft 365 [ou Office 365](https://support.office.com/article/Add-a-domain-to-Office-365-6383f56d-3d09-4dcb-9b41-b5f5a5efd611).
    
2. Atualize o arquivo de configuração do Cloud Connector com o novo domínio SIP ou domínios.
    
3. Solicite um novo certificado externo de Borda com nomes SAN adicionais para sip.domain para cada domínio SIP definido em sua configuração do Cloud Connector. 
...
5. 
    ...
    Remova o registro de locatário para cada dispositivo em um site executando o seguinte cmdlet no administrador do PowerShell no Cloud Connector:
...
6. 
    ...
    Remova o registro de site para cada site executando o seguinte cmdlet no Skype for Business Online PowerShell:
...
7. 
    ...
    Desinstale cada dispositivo executando o seguinte cmdlet no administrador do PowerShell no Cloud Connector:
...
8. 
     ...
     Registre cada dispositivo executando o seguinte cmdlet no administrador do PowerShell no Cloud Connector:
...
9. 
     ...
     Instale cada dispositivo, um por um, executando o seguinte cmdlet no administrador do PowerShell no Cloud Connector:
...
Quando precisar substituir o certificado de Borda externo em seus dispositivos do Cloud Connector, você precisará obter um novo certificado de Borda, preparar o arquivo PFX que contém a chave privada e a cadeia de certificados completa e, em seguida, fazer o seguinte em cada dispositivo:
