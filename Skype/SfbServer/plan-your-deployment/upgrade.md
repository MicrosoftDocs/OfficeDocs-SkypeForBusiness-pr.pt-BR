...
> [!NOTE]
> In-Place atualização não dá suporte a alta disponibilidade ou recuperação de desastres para o Skype for Business Server. 
  
A atualização in-locar envolve colocar o pool do Lync Server 2013 offline e atualizando-o para um pool do Skype for Business Server 2015. 
...
> [!NOTE]
> Ao atualizar um pool emparelhado no Lync Server 2013 e você deseja atualizar ambos os pools para o Skype for Business Server 2015. Certifique-se de atualizar o segundo pool imediatamente após a atualização do primeiro pool. Quando um pool estiver executando o Lync Server 2013 e o segundo pool estiver executando o Skype for Business Server 2015, as opções de recuperação de desastres serão minimizadas. Por exemplo, se um pool estiver executando 2013 e o segundo for 2015 e houver um desastre, você poderá ter perda de dados porque o failover do pool não é suportado no modo de desastre quando pools emparelhados não são a mesma versão. 
  
#### <a name="in-place-upgrade-offline-method"></a>Método Offline de atualização in-locar

Use este método se você não quiser mover usuários entre pools de usuários. Durante a atualização, os usuários não poderão usar os serviços do Lync ou do Skype for Business. 
...
Depois de começar a atualizar um pool de servidores, você deve concluir a atualização de todo o pool. O Skype for Business Server não dá suporte a ter apenas uma parte do pool atualizada. 
  
#### <a name="move-users-method-no-user-downtime"></a>Método Move Users (sem tempo de inatividade do usuário)
<a name="bkmk_MoveUsersMethod"> </a>

Para usar esse método, você move os usuários para outro pool antes de iniciar a atualização. Durante a atualização, os usuários podem usar os serviços do Lync. Depois que eles são movidos para o pool atualizado, eles podem usar o Skype for Business. O diagrama a seguir mostra uma visão geral desse processo.
...
### <a name="in-place-upgrade-process"></a>Processo de atualização in-loco
 ...
 Atualizar do Lync Server 2013 para o Skype for Business Server 2015 usando as seguintes etapas:
...
O Skype for Business Server 2015 não dá suporte a uma atualização In-Place de um Dispositivo de Filial (SBA) ou um Servidor de Filial (SBS) que sobrevive.
  
No entanto, suportamos a coexistência dos datacenters do Skype for Business Server com o Lync Server 2010 ou o Lync Server 2013 SBA/SBS. 
