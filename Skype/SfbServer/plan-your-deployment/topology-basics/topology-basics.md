...
**Resumo:** escolha sua topologia do Skype for Business Server 2015. Aprenda sobre a colocação de servidor do Skype for Business Server 2015.
  
Antes de preparar qualquer outra coisa, você vai querer saber que você estiver planejando para a topologia adequada para sua implantação do Skype para Business Server 2015. A primeira coisa que você precisará decidir é se você vai ter uma implantação local do Skype para Business Server 2015 ou se você vai combinar isso com um Skype para implantação Business Server Online em uma implantação híbrida. De qualquer forma, você vai desejamos ler Além disso, como podemos vai detalham as topologias de local aqui, mas os detalhes de híbrido documentados em sua própria seção.
  
Você também pode ver alguns exemplos de topologias em [topologias de referência para Skype para Business Server 2015](reference-topologies.md).
...
Skype para Business Server 2015 oferece suporte a implantação de local de um ou mais sites que podem ser dimensionados de acordo com suas necessidades de local e a alta disponibilidade.
...
- Aparelho de filial persistente, que combina um gateway PSTN (rede) telefônica pública comutada, com algumas Skype para a funcionalidade do Business Server 2015.
    
- Servidor de filial persistente, é um servidor executando o Windows Server que tenha Skype para Business Server 2015 registrador e o servidor de mediação software instalado.
...
- Office Web Apps Server, que é usado com o Office Web Apps no Skype para Business Server 2015 para compartilhamento e de renderização de apresentações do PowerPoint.
...
- Diretor ou pool de diretores. Não é necessária, mas útil se você deseja melhorar a resiliência e habilitar o redirecionamento do Skype para negócios 2015 solicitações de usuário para o pool base do usuário. Se você deseja implantar os diretores, há suporte para um máximo de 10 por pool. Caso se trate algo que você precisa, definitivamente continue lendo em Planning for tópico diretores.
    
- Proxy reverso. Isso não é um Skype para o componente de negócios Server 2015, mas se você quiser dar suporte ao compartilhamento de conteúdo da web para federados usuários, se você pretende oferecer suporte ao tráfego de mobilidade, se quiserem que os usuários remotos usar o catálogo de endereços, participar de reuniões e assim por diante, isso é algo Você vai querer ter em seu ambiente. Não há uma configuração de tópico de servidor de proxy reverso, que Confira para obter mais detalhes, quando você estiver pronto para cima.
...
Onde está o servidor do Exchange Unified Messaging (UM) nesta lista? Bem, você pode certamente usá-lo com Skype para Business Server 2015 se você deseja integrar com UM do Exchange, mas não é um componente do Skype para site de Business Server, portanto, podemos não estiver mencioná-lo aqui.
...
Assim como última lista, podemos não incluindo UM servidor Exchange aqui porque ele não faz parte do Skype para implantação Business Server 2015, mas ele enquadra aqui, a mesma categoria muito.
...
Para a maioria das funções de servidor, para escalabilidade e alta disponibilidade você pode implantar pools de vários servidores todos executando a mesma função de servidor. Cada servidor de um pool deve executar funções idênticas de servidor. Para a maioria dos tipos de pools em Skype para Business Server, você deve implantar um balanceador de carga para difundir o tráfego entre os vários servidores no pool. Skype para Business Server suporta o balanceamento de carga de sistema de nome de domínio (DNS) e balanceadores de carga de hardware.
...
- Opcionalmente, Arquivamento, para arquivar o conteúdo de reuniões e comunicações de IM por motivos de conformidade. Para obter detalhes, consulte [Planning for Archiving](http://technet.microsoft.com/library/898b83c1-007b-43be-9484-08fe49c10951.aspx) na documentação de planejamento.
...
Para obter detalhes, consulte o [componente de servidor de mediação em Skype para Business Server 2015](../../plan-your-deployment/enterprise-voice-solution/mediation-server.md).
  
### <a name="video-interop-server"></a>Servidor de Interoperabilidade de Vídeo

Servidor de interoperabilidade de vídeo é uma nova função no Skype para Business Server 2015. Ele permite que você integre sua Skype para implantação de servidor de negócios determinadas soluções VTC (Video Teleconferencing System) de terceiros. Um VIS atua como um intermediário entre um sistema de teleconferência de terceiros 3º e um Skype para implantação de servidor de negócios. Para essa versão, o VIS se concentra na interoperabilidade com os sistemas de vídeo da Cisco e da Tandberg.
  
Para obter detalhes, consulte [Planejar para o servidor de interoperabilidade de vídeo no Skype para Business Server 2015](../../plan-your-deployment/video-interop-server.md).
...
Para obter detalhes sobre o pareamento de pool e alta disponibilidade do servidor Back-End, consulte [Planejar a alta disponibilidade e recuperação de desastres em Skype para Business Server 2015](../../plan-your-deployment/high-availability-and-disaster-recovery/high-availability-and-disaster-recovery.md).
  
## <a name="server-collocation-in-skype-for-business-server-2015"></a>Posicionamento do servidor em Skype para Business Server 2015

Usamos o termo colocar já, mas o que isso significa? Skype para Business Server 2015 permite que você localize alguns recursos e funções de servidor no mesmo servidor, que é de colocação, ou em servidores diferentes, mas pode ser confusa quando você está iniciando check-out e se você estiver fazendo um Standard Edition ou Enterprise Edition implantação de servidor (cada um deles vêm com seus próprios regras). Para ajudar no seu planejamento, estamos incluindo colocação do servidor em implantações de servidor do Standard Edition e implantações de pool de Front End do Enterprise Edition (na maioria dos casos, essa informação é idêntica e onde ele está diferente, ele é chamado check-out especificamente).
...
Agora, há algumas regras adicionais a seguir ao colocando Skype para bancos de dados corporativos Server 2015 em uma única instância SQL ou em várias instâncias do SQL no mesmo banco de dados do SQL Server:
...
- Você não é possível colocar um servidor de proxy reverso, que não é um Skype para Business Server 2015 componente e talvez nem estejam em sua topologia. Se você quiser dar suporte ao compartilhamento de conteúdo da web para que os usuários federados, entre muitas outras coisas, você precisará um proxy reverso. Se for necessário, vá em frente e implementar o suporte de proxy reverso para Skype para Business Server 2015 Configurando um servidor de proxy reverso existente que já esteja em sua organização que está sendo usada por outros aplicativos.
    
- Você não é possível colocar nenhum componente de UM do Exchange ou o componente do SharePoint Server com qualquer Skype para função Business Server 2015.
