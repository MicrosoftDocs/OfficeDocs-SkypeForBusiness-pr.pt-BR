...
O Servidor de Mediação converte a sinalização entre sua infraestrutura interna do Enterprise Voice e um gateway PSTN (rede telefônica pública comutado) ou um tronco SIP( Session Initiation Protocol). Em algumas implantações, ela também converte a própria mídia entre esses pontos.
...
## <a name="enhanced-9-1-1-e9-1-1-and-mediation-server"></a>Enhanced 9-1-1 (E9-1-1) and Mediation Server
...
Também recomendamos que você collocate o Servidor de Mediação em um pool de Front-End quando tiver implantado o IP-PBXs ou se conectar a um SBC (Controlador de Borda de Sessão) do Provedor de Telefonia da Internet, desde que qualquer uma das seguintes condições seja atendida:
...
- Executar balanceamento de carga DNS (Sistema de Nomes de Domínio) de camada de rede nos Servidores de Mediação em um pool (ou rotear o tráfego uniformemente para todos os Servidores de Mediação em um pool)
