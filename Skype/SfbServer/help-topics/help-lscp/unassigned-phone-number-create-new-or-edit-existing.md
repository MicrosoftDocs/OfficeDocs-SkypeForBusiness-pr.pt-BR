...
- **Intervalo de números** No primeiro campo, digite o número inicial do intervalo de números não atribuídos. No segundo campo, digite o número final do intervalo.
  ...
  - O número deve corresponder à expressão regular `(tel:)?(\+)?[1-9]\d{0,17}(;ext=[1-9]\d{0,9})?`. Isso significa que `tel:` o número pode começar com a cadeia de caracteres (se você não especificar essa cadeia de caracteres será adicionada automaticamente para você), um sinal de a mais (+) e um dígito de 1 a 9. O número de telefone pode ter até 17 dígitos e pode ser seguido por uma extensão no formato ;ext= seguido pelo número de extensão.
