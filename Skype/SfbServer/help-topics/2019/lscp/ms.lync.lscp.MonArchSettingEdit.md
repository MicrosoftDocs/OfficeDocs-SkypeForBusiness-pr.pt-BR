...
> [!NOTE]
> As configurações de arquivamento se aplicam aos usuários que estão no Skype for Business Server e, se você habilitar Exchange opção de integração do Microsoft Exchange para usar o Exchange para armazenar dados de arquivamento no Microsoft Exchange, aos usuários que estão no Exchange. No entanto, algumas opções são implementadas de forma ligeiramente diferente para usuários Exchange, conforme descrito na próxima seção. 
...
- **Integração Exchange Microsoft** Selecione essa opção se você tiver usuários que estão no Exchange. Com essa opção, Exchange é usado para armazenar dados para esses usuários, se suas caixas de correio foram colocadas em In-Place Hold. Se todos os usuários estão Exchange, você não precisa configurar bancos de dados SQL Server separados para armazenamento de dados de arquivamento.
