...
Se a configuração de federação no servidor de borda ou pool tiver sido configurada, você pode configurar as seguintes opções: 
  
- **Atribuições de rota de federação de permitir a todos os sites** Essa configuração afetará todos os sites. Certifique-se de que a configuração que você está configurando neste site é apropriada para todos os sites.
