...
<span data-ttu-id="4a7ee-105">É necessário incorporar o arquivamento à sua topologia antes de poder configurar sua implantação para suportar o arquivamento.</span><span class="sxs-lookup"><span data-stu-id="4a7ee-105">You must incorporate archiving into your topology before you can configure your deployment to support archiving.</span></span> <span data-ttu-id="4a7ee-106">As informações neste tópico explicam como usar o Construtor de Topologias para:</span><span class="sxs-lookup"><span data-stu-id="4a7ee-106">The information in this topic explains how to use Topology Builder to:</span></span>
  
- <span data-ttu-id="4a7ee-107">Adicione um banco de dados de arquivamento à sua topologia.</span><span class="sxs-lookup"><span data-stu-id="4a7ee-107">Add an archiving database to your topology.</span></span>
...
> [!NOTE]
> <span data-ttu-id="4a7ee-109">Se você quiser usar a integração com o Microsoft Exchange para armazenar arquivos e dados de arquivamento em servidores do Exchange para todos os usuários em sua implantação, não especifique o armazenamento do **SQL Server** de Arquivamento ou use as informações de espelhamento do SQL **Server Store.**</span><span class="sxs-lookup"><span data-stu-id="4a7ee-109">If you want to use Microsoft Exchange integration to store archiving data and files on Exchange servers for all your users in your deployment, do not specify **Archiving SQL Server store** or **Use SQL Server Store mirroring** information.</span></span>
  
### <a name="add-an-archiving-database-to-your-topology"></a><span data-ttu-id="4a7ee-110">Adicionar um banco de dados de arquivamento à sua topologia</span><span class="sxs-lookup"><span data-stu-id="4a7ee-110">Add an archiving database to your topology</span></span>

1. <span data-ttu-id="4a7ee-111">Em um computador que está executando o Skype for Business Server ou no qual as ferramentas administrativas do Skype for Business Server estão instaladas, faça logoff usando uma conta que seja membro do grupo usuários local (ou uma conta com direitos de usuário equivalentes).</span><span class="sxs-lookup"><span data-stu-id="4a7ee-111">On a computer that is running Skype for Business Server, or on which the Skype for Business Server administrative tools are installed, log on by using an account that is a member of the local Users group (or an account with equivalent user rights).</span></span>
    
2. <span data-ttu-id="4a7ee-112">Inicie o Construtor de topologia.</span><span class="sxs-lookup"><span data-stu-id="4a7ee-112">Start Topology Builder.</span></span>
    
3. <span data-ttu-id="4a7ee-113">Na árvore de console, navegue até o pool de Front-End no qual você deseja implantar o Arquivamento e clique no nome do pool de Front-End onde deseja implantar o arquivamento.</span><span class="sxs-lookup"><span data-stu-id="4a7ee-113">In the console tree, navigate to the Front End pool in which you want to deploy Archiving, and then click the name of the Front End pool where you want to deploy archiving.</span></span>
    
4. <span data-ttu-id="4a7ee-114">No menu **Ação**, clique em **Editar Propriedades**.</span><span class="sxs-lookup"><span data-stu-id="4a7ee-114">In the **Action** menu, click **Edit Properties**.</span></span> 
    
5. <span data-ttu-id="4a7ee-115">Na caixa de diálogo **Editar Propriedades**, clique em **Geral**.</span><span class="sxs-lookup"><span data-stu-id="4a7ee-115">In the **Edit Properties** dialog box, click **General**.</span></span>
    
6. <span data-ttu-id="4a7ee-116">Role a tela para baixo até **Arquivamento**.</span><span class="sxs-lookup"><span data-stu-id="4a7ee-116">Scroll down to **Archiving**.</span></span>
    
7. <span data-ttu-id="4a7ee-117">Marque a caixa de seleção **Arquivamento**.</span><span class="sxs-lookup"><span data-stu-id="4a7ee-117">Select the **Archiving** check box.</span></span>
    
8. <span data-ttu-id="4a7ee-118">Em **Armazenamento de Arquivamento do SQL Server,** faça um dos seguintes:</span><span class="sxs-lookup"><span data-stu-id="4a7ee-118">Under **Archiving SQL Server store,** do one of the following:</span></span>
    
   - <span data-ttu-id="4a7ee-119">Para usar um armazenamento existente do SQL Server, na caixa de listagem suspensa, clique no nome do armazenamento do SQL Server que deseja usar.</span><span class="sxs-lookup"><span data-stu-id="4a7ee-119">To use an existing SQL Server store, in the drop-down list box, click the name of the SQL Server store that you want to use.</span></span> <span data-ttu-id="4a7ee-120">Se todos os seus usuários estão instalados no Microsoft Exchange Server 2013 ou acima, você pode arquivar comunicações do Skype for Business para todos os seus usuários no Exchange.</span><span class="sxs-lookup"><span data-stu-id="4a7ee-120">If all of your users are homed on Microsoft Exchange Server 2013 or above, you can archive Skype for Business communications for all your users in Exchange.</span></span> <span data-ttu-id="4a7ee-121">Nesse caso, você não precisa configurar o armazenamento de Arquivamento do SQL Server.</span><span class="sxs-lookup"><span data-stu-id="4a7ee-121">In this case, you don't need to configure SQL Server Archiving store.</span></span>
    
   - <span data-ttu-id="4a7ee-122">Para especificar um novo armazenamento do SQL Server, clique em **Novo** e, na caixa de diálogo Definir Novo Armazenamento do **SQL Server,** faça o seguinte:</span><span class="sxs-lookup"><span data-stu-id="4a7ee-122">To specify a new SQL Server store, click **New**, and then in the **Define New SQL Server Store** dialog box, do the following:</span></span>
    
   - <span data-ttu-id="4a7ee-123">No **FQDN do SQL Server,** especifique o FQDN do servidor no qual você deseja criar o novo armazenamento do SQL Server.</span><span class="sxs-lookup"><span data-stu-id="4a7ee-123">In **SQL Server FQDN**, specify the FQDN of the server on which you want to create the new SQL Server store.</span></span>
...
2. <span data-ttu-id="4a7ee-146">Abra a topologia criada na seção anterior usando o Construtor de Topologias.</span><span class="sxs-lookup"><span data-stu-id="4a7ee-146">Open the topology you created in the previous section using Topology Builder.</span></span>
    
3. <span data-ttu-id="4a7ee-147">Na árvore do console, clique com o botão direito do mouse **no Skype for Business Server** e clique em Publicar **Topologia.**</span><span class="sxs-lookup"><span data-stu-id="4a7ee-147">In the console tree, right-click **Skype for Business Server**, and then click **Publish Topology**.</span></span>
    
4. <span data-ttu-id="4a7ee-148">Na página **Publicar topologia**, clique em **Avançar**.</span><span class="sxs-lookup"><span data-stu-id="4a7ee-148">On the **Publish the topology** page, click **Next**.</span></span>
    
5. <span data-ttu-id="4a7ee-149">Na página **Criar banco de dados**, verifique se o banco de dados está selecionado e, então, clique em **Avançar**.</span><span class="sxs-lookup"><span data-stu-id="4a7ee-149">On the **Create databases** page, verify that the database is selected, and then click **Next**.</span></span> 
    
    > [!NOTE]
    > <span data-ttu-id="4a7ee-150">Se você não possuir as permissões adequadas para criar bancos de dados, você pode cancelar a seleção do banco de dados e alguém com as permissões adequadas podem criar o banco de dados.</span><span class="sxs-lookup"><span data-stu-id="4a7ee-150">If you do not have the appropriate permissions to create databases, you can cancel the selection of the database and someone with appropriate permissions can create the database.</span></span> <span data-ttu-id="4a7ee-151">> somente bancos de dados em SQL Servers dedicados podem ser instalados usando o Construtor de Topologias.</span><span class="sxs-lookup"><span data-stu-id="4a7ee-151">> Only databases on dedicated SQL Servers can be installed by using Topology Builder.</span></span> <span data-ttu-id="4a7ee-152">Os bancos de dados em Servidores SQL que são colocados com outros componentes precisam ser instalados executando a configuração local nesse computador.</span><span class="sxs-lookup"><span data-stu-id="4a7ee-152">Databases on SQL Servers that are collocated with other server components must be installed by running local setup on that computer.</span></span> 
  
6. <span data-ttu-id="4a7ee-153">Na página **Assistente de publicação concluído**, verifique se a topologia foi publicada com êxito e clique em **Concluir**.</span><span class="sxs-lookup"><span data-stu-id="4a7ee-153">On the **Publishing wizard complete** page, verify that the topology was successfully published, and then click **Finish**.</span></span>
    
    > [!IMPORTANT]
    > <span data-ttu-id="4a7ee-154">Após publicar a topologia, você deve configurar as opções e políticas de Arquivamento antes que qualquer conteúdo seja arquivado.</span><span class="sxs-lookup"><span data-stu-id="4a7ee-154">After publishing the topology, you must configure options and policies for Archiving before any content can be archived.</span></span> <span data-ttu-id="4a7ee-155">Para obter detalhes, [consulte Configure archiving options for Skype for Business Server](configure-archiving-options.md) and Configure [archiving policies for Skype for Business Server](configure-archiving-policies.md).</span><span class="sxs-lookup"><span data-stu-id="4a7ee-155">For details, see [Configure archiving options for Skype for Business Server](configure-archiving-options.md) and [Configure archiving policies for Skype for Business Server](configure-archiving-policies.md).</span></span> 
