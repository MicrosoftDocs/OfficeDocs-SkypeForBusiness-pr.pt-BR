...
As fotos de alta resolução, que são acessadas usando o Exchange Web Services, podem ser carregadas por usuários que executam o Outlook Web App 2013; os usuários só têm permissão para atualizar suas próprias fotos. Os administradores, no entanto, podem atualizar a foto para qualquer usuário usando o Shell de Gerenciamento Exchange e uma série de Windows PowerShell comandos semelhantes ao seguinte:
...
O primeiro comando no exemplo `Get-Content` anterior usa o cmdlet para ler o conteúdo do arquivo C:\Photos\Kenmyer.jpg e armazenar esses dados em uma variável chamada $photo. No segundo comando, o cmdlet `Set-UserPhoto` Exchange é usado para carregar a foto e anexar essa foto à conta de usuário de Ken Myer.
...
Observe também que nenhuma configuração adicional é necessária para disponibilizar essa foto Skype for Business. Em vez disso, a foto estará disponível instantaneamente depois que ela tiver sido carregada `Set-UserPhoto` e o cmdlet tiver sido executado.