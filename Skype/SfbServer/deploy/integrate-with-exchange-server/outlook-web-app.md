...
Para integrar o Skype for Business Server local ao Exchange Online, você deve configurar um espaço de endereço SIP compartilhado. O mesmo espaço de endereço do domínio SIP é suportado pelo Skype for Business Server e pelo serviço do Exchange Online.
...
Para obter detalhes sobre como usar o Shell de Gerenciamento do Skype for Business Server, consulte [o Shell de Gerenciamento do Skype for Business Server.](../../manage/management-shell.md)

## <a name="configure-a-hosting-provider-on-the-edge-server"></a>Configurar um provedor de hospedagem no Servidor de Borda

Usando o Shell de Gerenciamento do Skype for Business Server, configure um provedor de hospedagem no Servidor de Borda executando o cmdlet **New-CsHostingProvider,** usando os parâmetros do exemplo a seguir:
...
> [!NOTE]
> Se você estiver usando o Microsoft 365 ou o Office 365 operado pela 21Vianet na China, substitua o valor do parâmetro ProxyFqdn neste exemplo ("exap.um.outlook.com") pelo FQDN do serviço operado pela 21Vianet: "exap.um.partner.outlook.cn". Se você estiver usando o Microsoft 365 ou Office 365 GCC High, substitua o valor do parâmetro ProxyFqdn neste exemplo ("exap.um.outlook.com") pelo FQDN para GCC High: "exap.um.office365.us".

- **A** identidade especifica um identificador de valor de cadeia de caracteres exclusivo para o provedor de hospedagem que você está criando (por exemplo, "Exchange Online"). Valores que contêm espaços devem estar entre aspas duplas.
...
- **VerificationLevel** Indica o nível de verificação permitido para mensagens enviadas de e para o provedor hospedado. Especifique **UseSourceVerification**, que depende do nível de verificação incluído nas mensagens enviadas do provedor de hospedagem. Se esse nível não for especificado, a mensagem será rejeitada como sendo não-verificável.
...
As alterações feitas usando os cmdlets nas seções anteriores são aplicadas automaticamente ao Servidor de Borda e geralmente levam menos de um minuto para replicar. Você pode validar o status de replicação e confirmar se as alterações foram aplicadas ao seu Servidor de Borda usando os cmdlets a seguir.
...
```
Verifique se o valor UpToDate está mostrando VERDADEIRO para todas as réplicas.
...
```
Verifique se as informações mostradas coincidem com as alterações comprometidas nas etapas anteriores.

## <a name="see-also"></a>Confira também

[Fornecer correio de voz para usuários do Skype for Business Server na UM do Exchange hospedada](https://technet.microsoft.com/library/306d3fb5-231b-4f0b-b8d8-0d9083b5ed77.aspx)
