...
---
# <a name="deploy-skype-room-systems-v2-management-with-azure-monitor"></a>Implantar o gerenciamento de v2 Skype sala sistemas com Monitor do Azure

Este artigo discute como configurar e implantar o gerenciamento integrado de ponta a ponta de dispositivos do Skype sala sistemas v2 usando o Monitor do Azure.
...
Seguindo este guia, você pode usar um painel semelhante ao seguinte exemplo para obter o status detalhado de relatórios para disponibilidade de dispositivo, aplicativo e integridade de hardware e aplicativos do Skype sala sistemas v2 e distribuição de versão do sistema operacional.

![Modo de exibição de análise de Log de amostra para v2 SRS] (../../media/Deploy-Azure-Monitor-1.png "Modo de exibição de análise de Log de amostra para v2 SRS")
...
4.  [Definir os modos de exibição do Skype sala sistemas v2 na análise de Log](azure-monitor.md#Define_Views)
...
> [!IMPORTANT]
> Embora com a configuração mínima, análise de Log do Windows Azure Monitor pode monitorar a um computador que executa um sistema operacional Windows, ainda há algumas etapas específicas v2 Skype sala sistemas que precisam ser executadas antes de começar a implantar agentes para todos os sistemas de sala do Skype dispositivos.
...
Você precisa ter um espaço de trabalho de análise de Log para iniciar a coleta de logs de dispositivos do Skype sala sistemas v2. Um espaço de trabalho é um ambiente de análise de Log exclusivo com seu próprio repositório de dados, fontes de dados e soluções. Se você já tiver um espaço de trabalho de análise de Log existente, você pode usá-lo para monitorar sua implantação do Skype sala sistemas v2 ou como alternativa, você pode criar um espaço de trabalho de análise de Log dedicado específica para suas necessidades de monitoramento v2 Skype sistemas de sala.
...
### <a name="configure-log-analytics-to-collect-skype-room-systems-v2-event-logs"></a>Configurar o Log de análise para coletar logs de eventos do Skype sala sistemas v2
...
Você precisa configurar o Log de análise para coletar os logs necessários para monitorar o status de dispositivo e aplicativo de sistemas de sala Skype v2. Dispositivos de v2 de sistemas de sala Skype usam o log de eventos do **Sistema do Skype sala** .

Para configurar a análise de Log para coletar os eventos de sistemas de sala Skype v2, consulte [fontes de dados de log de eventos do Windows no Monitor do Azure](https://docs.microsoft.com/azure/azure-monitor/platform/data-sources-windows-events)
...
Você precisará preparar a análise de Log para poder monitorar eventos relacionados ao v2 do Skype sistemas de sala. Para começar com, você precisa implantar o Microsoft Monitoring agentes a apenas um ou dois dispositivos de v2 Skype sala sistemas que você tem acesso físico ao e obter os dispositivos de teste gerar alguns dados e push-lo no espaço de trabalho de análise de Log.
...
> [!NOTE]
> Reinicializar o dispositivo após a instalação do Microsoft Monitoring agent e verifique se que esse aplicativo de reunião do Skype sala sistemas v2 foi iniciado, para que ele possa gerar novos eventos no Log de eventos.

1.  Entrar no [portal do Microsoft Azure](https://portal.azure.com) e vá para a análise de Log e selecione sua área de trabalho.

2.  Liste os eventos de pulsação gerados por um dispositivo do Skype sala sistemas v2:
    1.  Selecione sua área de trabalho e vá para **Logs** e usar uma consulta para recuperar os registros de pulsação que terão os campos personalizados para SRS v2.
    2.  Consulta de exemplo:`Event | where Source == "SRS-App" and EventID == 2000`

3.  Certifique-se de que a consulta retorna registros de log que incluem os eventos gerados pelo sistemas de sala Skype v2 reuniões app.
...
2. Liste os eventos gerados por um dispositivo do Skype sala sistemas v2:
   1.  Vá para **Logs (clássico)** e usar uma consulta para recuperar os registros que terão o campo personalizado.
   2.  Consulta de exemplo:`Event | where Source == "SRS-App" and EventID == 2000`

3. Selecione um dos registros, selecione o botão à esquerda e iniciar o Assistente de extração de campo.

   ![Assistente de extração de campo] (../../media/Deploy-Azure-Monitor-3.png "Assistente de extração de campo")
...
|:---------------------------------|:-------------------------------|:-------------|:-------------------------------------------------------|
| Descrição                      | SRSEventDescription         | US$ 2.000     | Evento \| onde de origem = = "SRS-App" e EventID = = 2000 |
| ResourceState                    | SRSResourceState            | US$ 2.000     | Evento \| onde de origem = = "SRS-App" e EventID = = 2000 |
| OperationName                    | SRSOperationName            | US$ 2.000     | Evento \| onde de origem = = "SRS-App" e EventID = = 2000 |
| OperationResult                  | SRSOperationResult          | US$ 2.000     | Evento \| onde de origem = = "SRS-App" e EventID = = 2000 |
| OS                               | SRSOSVersion                | US$ 2.000     | Evento \| onde de origem = = "SRS-App" e EventID = = 2000 |
| Versão do sistema operacional                        | SRSOSLongVersion            | US$ 2.000     | Evento \| onde de origem = = "SRS-App" e EventID = = 2000 |
| Alias                            | SRSAlias                    | US$ 2.000     | Evento \| onde de origem = = "SRS-App" e EventID = = 2000 |
| DisplayName                      | SRSDisplayName              | US$ 2.000     | Evento \| onde de origem = = "SRS-App" e EventID = = 2000 |
| AppVersion                       | SRSAppVersion               | US$ 2.000     | Evento \| onde de origem = = "SRS-App" e EventID = = 2000 |
| IPv4Address                      | SRSIPv4Address              | US$ 2.000     | Evento \| onde de origem = = "SRS-App" e EventID = = 2000 |
| IPv6Address                      | SRSIPv6Address              | US$ 2.000     | Evento \| onde de origem = = "SRS-App" e EventID = = 2000 |
...
## <a name="define-the-skype-room-systems-v2-views-in-log-analytics"></a>Definir os modos de exibição do Skype sala sistemas v2 na análise de Log
...
Você pode importar um painel do Skype sala sistemas v2 e iniciar rapidamente seus dispositivos de monitoramento. Siga as etapas a seguir para importar o painel:
...
Monitor Azure pode disparar alertas para notificar os administradores, quando um console do Skype sala sistemas v2 encontra um problema.
...
Configure uma regra de alerta que verifica os sistemas de sala Skype v2 dispositivos que foram encontrados problemas de hardware na última hora.
1.  Entrar no [portal do Microsoft Azure](https://portal.azure.com) e vá para a análise de Log e selecione sua área de trabalho.
...
<a name="configure_all_devices"></a> Após os painéis e os alertas são configuradas, você pode configurar e configurar o Microsoft Monitoring agent em todos os dispositivos de sistemas de sala Skype v2 para concluir sua implantação de monitoramento.
...
Se você estiver criando seus dispositivos de sistemas de sala Skype v2 pela primeira vez, convém incluir as etapas de instalação e configuração de agente Monitoring Microsoft como parte do seu processo de criação. Para obter mais informações, consulte [instalar o agente usando a linha de comando](https://docs.microsoft.com/azure/azure-monitor/platform/agent-windows#install-the-agent-using-the-command-line).
...
Se você já implantou os dispositivos do Skype sala sistemas v2 antes de implementar o monitoramento do Azure, você pode usar o script fornecido para instalar e configurar os agentes usando objetos de diretiva de grupo do Active Directory.
