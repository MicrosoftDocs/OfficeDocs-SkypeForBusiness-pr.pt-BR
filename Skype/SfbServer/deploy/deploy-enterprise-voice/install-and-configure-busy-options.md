...
Se as Opções de Ocupado estiver habilitada para a organização, todos os usuários da empresa, tanto os usuários do Enterprise Voice quanto os que não são do Enterprise Voice, poderão usar as seguintes opções de configuração:
...
3. Execute o SkypeServerUpdateInstaller.exe de filial em cada Servidor de Filial Survivável (SBS), se quiser garantir o suporte para failover no SBS.
...
1. Execute o cmdlet [Set-CsVoicePolicy](https://docs.microsoft.com/powershell/module/skype/set-csvoicepolicy?view=skype-ps) para habilitar globalmente Opções de Ocupado, conforme mostrado no exemplo a seguir:

   ```powershell
   Set-CsVoicePolicy -EnableBusyOptions $true
   ```

2. Em seguida, se o site tiver uma política de voz, você deverá habilitar o recurso Opções de Ocupado para a política de voz da seguinte forma:
...
4. Em seguida, execute o cmdlet [Update-CsAdminRole](https://docs.microsoft.com/powershell/module/skype/update-csadminrole?view=skype-ps) para atualizar as funções do controle de acesso baseado em função (RBAC) para os cmdlets Opções de Ocupado, conforme mostrado no exemplo a seguir:
...
Por exemplo, o comando a seguir configura opções de ocupado para o usuário "Ken Myer". Nessa configuração, qualquer chamada para "Ken Myer" retornará um sinal de ocupado quando ele já estiver em uma chamada:

```powershell
Set-CsBusyOptions -Identity "Ken Myer"  -ActionType BusyOnBusy
```

No próximo exemplo, o comando configura as opções de ocupado para a usuária "Chrystal Next website". Nesta configuração, novas chamadas de entrada para "ChrystalIa" serão encaminhadas para a caixa postal quando ela já estiver em uma chamada:
...
Para habilitar o registro em log para Opções de Ocupado usando o Serviço de Log Centralizado, especifique o seguinte:
...
Você também pode usar o Visualizador de Eventos do Windows para verificar se a instalação de Opções de Ocupado foi bem-sucedida e se o Skype for Business Server carregou com êxito as Opções de Ocupado. Para verificar As Opções de Ocupado, abra o Visualizador de Eventos - Logs de Aplicativos e Serviços **- Servidor do Skype \> \> (ou Lync)** e pesquise a ID do Evento = 30253.
