...
**Minhas tarefas** mostra as tarefas individuais de um usuário. **Planos compartilhados** mostram tarefas nas quais toda a equipe está trabalhando e inclui qualquer lista de tarefas adicionada como uma guia do Tasks a um canal. Observe as seguintes relações entre tarefas em Tarefas, Para Fazer e Planejador:
...
2. Na lista de aplicativos, siga um destes procedimentos:
    ...
    - Para desativar Tarefas para sua organização, pesquise o aplicativo Tarefas, selecione-o e selecione **Bloquear**.
    - Para ativar Tarefas para sua organização, pesquise o aplicativo Tarefas, selecione-o e selecione **Permitir**.
...
Para permitir ou bloquear usuários específicos em sua organização de usar Tarefas, verifique se Tarefas estão ativadas para sua organização na página [Gerenciar aplicativos](manage-apps.md) e, em seguida, crie uma política personalizada para permissões de aplicativo e atribua-a a esses usuários. Para saber mais, confira [Gerenciar políticas de permissão de aplicativos no Teams](teams-app-permission-policies.md).
...
Para fixar o aplicativo Tarefas para seus usuários, você pode editar a política global (padrão em toda a organização) ou criar e atribuir uma política personalizada na política de configuração do aplicativo. Para saber mais, confira [Gerenciar políticas de configuração de aplicativos no Teams](teams-app-setup-policies.md).
...
Se não quiser que um usuário veja **Minhas tarefas**, você pode ocultá-lo. Para ocultar **Minhas tarefas**, [remova a licença de Exchange Online do usuário](/microsoft-365/admin/manage/remove-licenses-from-users). É importante saber que, depois de remover uma licença do Exchange Online, o usuário não tem mais acesso à caixa de correio.  Os dados da caixa de correio são retidos por 30 dias, após os quais os dados serão removidos e não podem ser recuperados, a menos que a caixa de correio seja colocada em [Bloqueio In-loco ou Retenção de Litígio ](/exchange/security-and-compliance/in-place-and-litigation-holds).

Não recomendamos remover uma licença Exchange Online para trabalhadores de informações, mas pode haver alguns cenários em que você pode ocultar **minhas tarefas** dessa maneira, como para trabalhadores da linha de frente que não dependem de email.
...
A equipe de Marketing compartilha os detalhes da promoção e a lista de tarefas correspondente com o Gerente de Comunicações de Varejo. O Retail Communications Manager, que atua como gatekeeper para lojas, examina as informações. Em seguida, eles criam uma lista de tarefas para a promoção e criam uma tarefa para cada unidade de trabalho que os repositórios afetados precisam executar. Quando a lista de tarefas for concluída, eles precisam selecionar os repositórios que devem concluir o trabalho. Nesse caso, a promoção só se aplica a lojas nos Estados Unidos que tenham um restaurante interno. Em Tarefas, eles filtram a lista de repositórios com base no atributo de restaurante na loja, selecionam os locais de Estados Unidos correspondentes na hierarquia e, em seguida, publicam a lista de tarefas nesses repositórios.
