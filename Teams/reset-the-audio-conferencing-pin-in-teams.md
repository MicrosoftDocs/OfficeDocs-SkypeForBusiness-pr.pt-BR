...
Um PIN é um código composto por números criados para cada usuário do Microsoft Teams habilitado para audioconferência. Os PINs de audioconferência são usados pelos organizadores da reunião para identificar que eles são o organizador da reunião e permitir que eles iniciem uma reunião por telefone. Se eles usarem o aplicativo Microsoft Teams para iniciar a reunião, um PIN não será necessário. Se os usuários esquecerem o PIN e não puderem encontrá-lo no email que foi enviado para eles quando foram habilitados para audioconferência, um administrador poderá redefinir o PIN ou redefinir seu próprio PIN.
  
As reuniões podem ser iniciadas quando um usuário autenticado ingressa usando o aplicativo Microsoft Teams ou quando o organizador ingressa com o PIN por telefone. Quando uma reunião exigir que um PIN seja iniciado, os usuários que ingressarem por telefone serão colocados no lobby e ouvirão música em espera até que o organizador os admita. Se o organizador de uma reunião não exigir um PIN para iniciar a reunião por telefone, os chamadores não precisarão fornecer um PIN para participar.
...
## <a name="have-a-user-reset-their-own-pin"></a>Fazer com que um usuário redefina seu próprio PIN

1. Fazer com que o usuário vá para [https://dialin.teams.microsoft.com/usp](https://dialin.teams.microsoft.com/usp).
...
> [!NOTE]
> Um email de redefinição de PIN não será enviado ao usuário se estiver usando esse método.
...
- Por padrão, o email será definido como o microsoft 365 ou Office 365 endereço SMTP primário do usuário. Você pode enviar um email para um endereço de email não Microsoft 365 ou não Office 365, como um endereço de email hotmail ou MSN. Você pode substituir o endereço de email padrão usando o Windows PowerShell. Isso será útil se os usuários não tiverem uma caixa de correio do Exchange no Microsoft 365 ou Office 365.
