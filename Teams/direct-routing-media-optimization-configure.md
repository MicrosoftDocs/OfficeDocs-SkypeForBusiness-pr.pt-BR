...
Este artigo descreve a configuração para componentes da Microsoft. Para obter informações sobre a configuração do SBC, consulte a documentação do fornecedor SBC.

A Otimização de Mídia Local é suportada pelos seguintes fornecedores SBC:
...
Observe o seguinte: 
   ...
   - Todos os parâmetros são sensíveis a minúsculas, portanto, você precisa garantir que você use o mesmo caso usado durante a instalação.  (Por exemplo, os valores gatewaySiteID "Vietnã" e "vietnã" serão tratados como sites diferentes.)
