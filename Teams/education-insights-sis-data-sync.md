...
|:--- |:--- |:--- |
| **Usuários** |Suporta apenas a função de 'educador', como resultado, as permissões em nível de organização para seus líderes educacionais precisam ser definidas manualmente|Suporta múltiplas funções para que as permissões baseadas em funções possam ser definidas|
...
- Certifique-se de **refletir a hierarquia real e completa da sua organização**. Em alguns casos, essa hierarquia não é refletida no SIS; nesse caso, ela precisa ser adicionada manualmente ao arquivo CSV antes da sincronização.
