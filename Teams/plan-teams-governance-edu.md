---
title: Perguntas frequentes sobre o Microsoft Education para administradores de TI
author: SerdarSoysal
ms.author: serdars
manager: serdars
ms.topic: article
...
---
# <a name="microsoft-education-faq-for-it-admins"></a>Perguntas frequentes sobre o Microsoft Education para administradores de TI
...
Em geral, recomendamos impedir que qualquer pessoa crie equipes. Se todos puderem criar equipes, é mais provável que o Teams seja amplamente adotado. Docentes, professores ou alunos podem usar o Teams para criar grupos de estudo ou grupos de interesse especial. Essa funcionalidade ajuda o Teams a ser aceito dentro e fora da sala de aula.
...
No início de cada semestre ou trimestre, você precisará de novas equipes. Pode fazer sentido usar uma abordagem automatizada para criar essas equipes automaticamente, preenchê-las com os usuários certos e definir as permissões corretas:

- O School Data Sync pode criar um Grupos do Microsoft 365 para o Exchange Online e o SharePoint Online, equipes de classe para blocos de anotações do Microsoft Teams e classe do OneNote, grupos escolares para o Intune for Education e integração de SSO (single Sign-On) para muitos outros aplicativos de terceiros. Saiba mais em Visão [geral do School Data Sync](/schooldatasync/overview-of-school-data-sync).
- Com o PowerShell, você pode criar equipes e canais e definir configurações automaticamente. Para obter mais informações, consulte [o Microsoft Teams PowerShell](/powershell/module/teams/?view=teams-ps).
- Você pode usar o Microsoft API do Graph (atualmente em versão prévia) para criar, configurar, clonar e arquivar equipes. Para obter mais informações, [consulte Usar o Microsoft API do Graph para trabalhar com o Microsoft Teams](/graph/api/resources/teams-api-overview).
...
- **Política de retenção:** Use esta política para excluir todos os dados com mais de uma idade especificada para garantir que os dados antigos sejam removidos de chats (para todos ou alguns usuários) e canais. Você também pode configurar o Teams para reter o conteúdo para que ele não possa ser excluído. Para obter mais informações, consulte [Políticas de retenção para o Microsoft Teams](https://techcommunity.microsoft.com/t5/Microsoft-Teams-Blog/Retention-policies-for-Microsoft-Teams/ba-p/178011).
- **Política de expiração:** Configure as equipes para expirar após um dia definido. Trinta dias antes da expiração, todos os proprietários de uma equipe são notificados de que sua equipe precisa ser renovada, caso contrário, ela será excluída. Embora um administrador possa recuperar equipes excluídas por mais 30 dias. Essa configuração é útil para garantir que as equipes não utilizadas sejam desativadas. Saiba mais na [Política de Expiração do grupo do Microsoft 365](https://support.office.com/article/office-365-group-expiration-policy-8d253fe5-0e09-4b3c-8b5e-f48def064733).

- **Equipe de arquivos:** Essa configuração coloca as equipes no modo somente leitura. Eles ainda podem ser pesquisados e pesquisados, mas ninguém pode adicionar novas postagens. [Arquivar ou restaurar uma equipe](https://support.office.com/article/archive-or-restore-a-team-dc161cfd-b328-440f-974b-5da5bd98b5a7) descreve como os proprietários da equipe podem arquivar uma equipe; Os proprietários da equipe também podem usar [o API do Graph (versão prévia)](/graph/api/resources/teams-api-overview) para arquivar ou restaurar uma equipe.
...
Sim. Os usuários podem  selecionar Criar Equipe do modelo existente ao criar uma nova equipe, e os proprietários do Teams também podem usar o [API do Graph (](/graph/api/resources/teams-api-overview)versão prévia) para criar uma nova equipe com base nos modelos disponíveis.
...
O [Microsoft API do Graph (versão prévia)](/graph/api/resources/teams-api-overview) pode realizar as seguintes tarefas:
...
- Dê ao usuário um link para a equipe que você criou.
...
[O PowerShell](/powershell/module/teams/?view=teams-ps) pode realizar as seguintes tarefas:
...
> [!TIP]
> Os API do Graph e os cmdlets do PowerShell estão constantemente adicionando funcionalidades. Verifique com frequência os artigos [do Microsoft API do Graph (](/graph/api/resources/teams-api-overview)versão prévia) e do [PowerShell](/powershell/module/teams/?view=teams-ps) para aprimoramentos de recursos.  
...
Para obter mais informações sobre as políticas do Teams, [consulte Gerenciar as configurações do Microsoft Teams para sua organização](enable-features-office-365.md).
