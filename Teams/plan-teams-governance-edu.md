...
Para evitar nomes inadequados ou enganoso ou apenas para fornecer mais estrutura para a forma como as equipes são nomeadas, você pode usar a política de nomenlagem de grupos do Microsoft 365 (no momento na visualização):
...
-   **Classificação** Você pode criar classificações que os usuários em sua organização podem definir quando eles criam um Grupo do Microsoft 365. 

> [!IMPORTANT]
> Usar a Política de Nomenização de Grupos do Microsoft 365 exige licenças do Azure Active Directory Premium P1 ou licenças do Azure AD Basic EDU para cada usuário exclusivo que seja membro de um ou mais grupos do Microsoft 365.

Para obter instruções detalhadas, consulte [Política de nomeniso de grupos do Office.](https://support.office.com/article/office-365-groups-naming-policy-6ceca4d3-cad1-4532-9f0f-d469dfbbb552)

> [!Note]
> Se as equipes são criadas automaticamente usando a entrada de outro sistema (por exemplo, Sincronização de Dados escolares), verifique se os dados de entrada estão em conformidade com a política de nomenisco que você configurou; se isso não ocorrer, a criação de equipe falhará.
...
Em geral, recomendamos evitar que qualquer pessoa tenha criado equipes. Se todos puderem criar equipes, é mais provável que o Teams seja amplamente adotado. Professores, professores ou alunos podem usar o Teams para criar grupos de estudo ou grupos de interesse especial. Isso ajudará o Teams a ser aceito dentro e fora da sala de aula.

Em nossa experiência, a educação do usuário ajuda a garantir o uso responsável do Teams. Assim que os usuários entenderem que a criação de equipes não é anônima, eles entendem as implicações da criação descuidadamente e tendem a se afastar da utilização incorretamente da ferramenta.
...
-   A Sincronização de Dados Escolares pode criar grupos do Microsoft 365 para Exchange Online e SharePoint Online, equipes de classe para blocos de anotações do Microsoft Teams e classe do OneNote, grupos escolares para o Intune para Educação e integração de lista e SSO (SSO) para muitos outros aplicativos de terceiros. Saiba mais em [Visão geral da sincronização de dados escolares.](/schooldatasync/overview-of-school-data-sync)
-   Com o PowerShell, você pode criar equipes e canais e configurar configurações automaticamente. Confira [o Microsoft Teams PowerShell](/powershell/module/teams/?view=teams-ps) para obter mais informações.
-   Você pode usar a API do Microsoft Graph (atualmente em beta) para criar, configurar, clonar e arquivar equipes. Consulte [Usar a API do Microsoft Graph para trabalhar com o Microsoft Teams](/graph/api/resources/teams-api-overview) para obter mais informações.

> [!TIP]
> A Sincronização de Dados escolares cria um Grupo [](https://techcommunity.microsoft.com/t5/School-Data-Sync/HiddenGroupMembershipEnabled-SDS-setting/td-p/159945) do Microsoft 365 para cada classe sincronizada e habilita a associação de grupo oculta para que apenas professores e alunos da classe possam ver os membros dessa classe. Se usar um processo diferente para criar grupos de classe, use o parâmetro HiddenGroupMembershipEnabled do cmdlet New-UnifiedGroup para atender aos mesmos requisitos de privacidade.
...
-   **Política de retenção:** Use isso para excluir todos os dados mais antigos do que uma idade especificada para garantir que os dados antigos são removidos de chats (para todos ou alguns usuários) e canais. Você também pode configurar o Teams para reter o conteúdo para que ele não possa ser excluído. Para obter mais informações, consulte [Políticas de retenção para o Microsoft Teams](https://techcommunity.microsoft.com/t5/Microsoft-Teams-Blog/Retention-policies-for-Microsoft-Teams/ba-p/178011).
...
-   **Equipe de arquivamento:** Essa configuração coloca as equipes no modo somente leitura. Eles ainda podem ser navegados e pesquisados, mas ninguém pode adicionar novas postagens. [Arquivar ou restaurar uma equipe](https://support.office.com/article/archive-or-restore-a-team-dc161cfd-b328-440f-974b-5da5bd98b5a7) descreve como os proprietários de equipe podem arquivar uma equipe; Os proprietários de equipe também podem usar [a API do Graph (beta)](/graph/api/resources/teams-api-overview) para arquivar ou restaurar uma equipe.
...
Sim. Os usuários podem selecionar **Criar** Equipe a partir do modelo existente ao criar uma nova equipe, e os proprietários do Teams também podem usar a API do [Graph (beta)](/graph/api/resources/teams-api-overview) para criar uma nova equipe a partir dos modelos disponíveis.
...
> [!TIP]
> A API do Graph e os cmdlets do PowerShell estão constantemente adicionando funcionalidade. Verifique frequentemente os artigos [da API do Microsoft Graph (beta)](/graph/api/resources/teams-api-overview) e do [PowerShell](/powershell/module/teams/?view=teams-ps) para aprimoramentos de recursos.  
...
Para obter mais detalhes sobre as políticas do Teams, consulte [Manage Microsoft Teams settings for your organization](enable-features-office-365.md).
...
-   Use a whitelisting de domínio para permitir ou bloquear convidados com base em seu domínio.
...
Para obter mais informações, consulte [Acesso de convidados em Grupos do Microsoft 365](https://support.office.com/article/Guest-access-in-Office-365-Groups-bfc7a840-868f-4fd6-a390-f347bf51aff6#PickTab=Manage).
...
## <a name="teams-evolves-so-quickly-how-can-i-stay-up-to-date"></a>O Teams evolui tão rapidamente. Como posso me manter atualizado?
