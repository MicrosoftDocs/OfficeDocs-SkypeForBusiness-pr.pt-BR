...
> [!Tip]
> Assista à sessão a seguir para saber mais sobre gerenciamento em Microsoft Teams: [Governança, gerenciamento e ciclo de vida em Microsoft Teams](https://aka.ms/teams-governance)
...
-   **Palavras bloqueadas personalizadas** Você pode carregar um conjunto de palavras que os usuários em uma organização específica são impedidos de usar em nomes de equipes que eles criam. Por exemplo, você pode bloquear os termos **CEO**, **Folha de Pagamento** e **RH** de serem usados em nomes de equipe para grupos aos que não se aplicam.
...
> [!Note]
> Se as equipes são criadas automaticamente usando a entrada de outro sistema (por exemplo, School Data Sync), verifique se os dados de entrada estão em conformidade com a política de nomenisão que você configurou; se isso não ocorrer, a criação de equipe falhará.
...
Se você tem certeza de que deseja controlar quem pode criar equipes, consulte [Gerenciar quem pode criar Microsoft 365 Grupos](https://support.office.com/article/manage-who-can-create-office-365-groups-4c46c8cb-17d0-44b5-9776-005fced8e618).
...
> [!TIP]
> School Data Sync cria um grupo Microsoft 365 para cada classe sincronizada e habilita a associação de grupo [](https://techcommunity.microsoft.com/t5/School-Data-Sync/HiddenGroupMembershipEnabled-SDS-setting/td-p/159945) oculta para que apenas professores e alunos da classe possam ver os membros dessa classe. Se usar um processo diferente para criar grupos de classe, use o parâmetro HiddenGroupMembershipEnabled do cmdlet New-UnifiedGroup para atender aos mesmos requisitos de privacidade.
...
Sim. Os usuários podem  selecionar Criar Equipe a partir do modelo existente ao criar uma nova equipe, e os proprietários do Teams também podem usar a [API de Graph (beta)](/graph/api/resources/teams-api-overview) para criar uma nova equipe a partir dos modelos disponíveis.
...
A [API Graph microsoft (beta)](/graph/api/resources/teams-api-overview) pode fazer o seguinte:
...
Para obter mais informações, consulte [Pesquisar o log de auditoria em busca de eventos Microsoft Teams](audit-log-events.md).
