...
Se você estiver considerando uma implementação de QoS, você já deve ter determinado seus requisitos de largura de banda e outros [requisitos de rede](prepare-network.md). Os cálculos de largura de banda do Microsoft Teams são complexos e para ajudar com isso, uma calculadora foi criada. Para acessar a calculadora, vá para o planejador de [rede](https://aka.ms/bwcalc/) no myadvisor.
...
> [!NOTE]
> Os usuários remotos conectados à VPN devem implementar o tunelamento dividido para maximizar a qualidade da experiência do usuário. O documento [implantação – orientação-VPN Split Tunnel](https://myadvisor.fasttrack.microsoft.com/CloudVoice/Downloads?SelectedIDs=5_1_0_9 ) está disponível no myadvisor e tem mais informações.
...
Ao implementar o QoS, você define várias filas usando um dos vários recursos de gerenciamento de congestionamento (como a enfileiramento de prioridade da Cisco e os recursos de impedimento com base em classe ponderado baseados em classe) e os recursos de impedimento de congestionamento (como ponderada aleatória no início [](https://www.cisco.com/en/US/docs/ios/12_0t/12_0t5/feature/guide/cbwfq.html#wp17641) [WRED](https://en.wikipedia.org/wiki/Weighted_random_early_detection)de detecção).
...
Usando esse método, o roteador da sua rede examina um pacote de entrada e, se o pacote chegou usando uma determinada porta ou intervalo de portas, ele o identifica como um determinado tipo de mídia e coloca-o na fila desse tipo, adicionando uma marca [DSCP](https://tools.ietf.org/html/rfc2474) predeterminada ao IP Cabeçalho do pacote para que outros dispositivos possam reconhecer seu tipo de tráfego e dar prioridade na fila.
...
4. Implementar as configurações de QoS:
   ...
   3. No [centro de administração](meeting-settings-in-teams.md#set-how-you-want-to-handle-real-time-media-traffic-for-teams-meetings) do teams
...
> [!NOTE]
> Se você já configurou o QoS com base em intervalos de porta de origem e marcações DSCP para o Skype for Business Online, a mesma configuração será aplicada às equipes e não será necessária nenhuma outra alteração de cliente ou de rede para o mapeamento, mas talvez seja necessário [definir os intervalos usado no centro de administração](meeting-settings-in-teams.md#set-how-you-want-to-handle-real-time-media-traffic-for-teams-meetings) do teams para coincidir com o que foi configurado para o Skype for Business online.
