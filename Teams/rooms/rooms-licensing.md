---
title: Suporte às Salas do Microsoft Teams
ms.author: dstrome
author: dstrome
manager: serdars
ms.reviewer: sohailta
ms.topic: article
ms.tgt.pltfrm: cloud
ms.service: msteams
search.appverid: MET150
ms.collection:
- M365-collaboration
- Teams_ITAdmin_Rooms
audience: Admin
appliesto:
- Microsoft Teams
ms.localizationpriority: medium
f1.keywords:
- NOCSH
ms.custom:
- Licensing
- LIL_Placement
- seo-marvel-apr2020
description: Saiba mais e compare as licenças disponíveis para diferentes tipos de recursos de chamada e reunião Salas do Microsoft Teams.
ms.openlocfilehash: af5f30f1a4ae0d34421629ffec3c67a619798b50
ms.sourcegitcommit: fc87f4300f53abf7a049936944abb21d0cade0d9
ms.translationtype: MT
ms.contentlocale: pt-BR
ms.lasthandoff: 10/06/2022
ms.locfileid: "68480481"
---
# <a name="microsoft-teams-rooms-licenses"></a>Suporte às Salas do Microsoft Teams

A Microsoft oferece duas licenças para Salas do Teams que participam de reuniões e chamadas do Teams:

- **Salas do Microsoft Teams Pro** oferece experiências de reunião na sala aprimoradas, como áudio e vídeo inteligente, linhas de frente e galerias grandes e suporte a tela dupla. A Salas do Teams Pro também fornece recursos avançados de gerenciamento, como gerenciamento de dispositivo remoto, políticas de acesso condicional e análise detalhada de dispositivos.

    Salas do Teams Pro é uma ótima opção para organizações médias e empresariais, bem como organizações menores com contagens de sala maiores ou necessidades mais avançadas. Salas do Teams pro podem ser usadas para licenciar sistemas Salas do Teams certificados e painéis do Teams.

- **Salas do Microsoft Teams Basic fornece** experiências de reunião principais para organizações que compram um sistema Salas do Microsoft Teams certificado, sem custo adicional. A Salas do Teams Básica inclui agendamento, ingresso em reuniões, compartilhamento de conteúdo e integração colaborativa em branco, bem como recursos básicos de segurança e gerenciamento imediatos.

    Uma Salas do Teams Basic pode ser usada para licenciar um único sistema Salas do Teams certificado em uma sala. Se você quiser fazer logon em mais de um Salas do Teams em uma sala usando a mesma conta de recurso, precisará usar uma licença Salas do Teams Pro.

    Você pode atribuir até 25 licenças Salas do Microsoft Teams Basic para Salas do Teams sistemas em sua organização. Se você precisar licenciar mais de 25 sistemas Salas do Teams, essas licenças adicionais precisarão ser Salas do Teams Pro. Salas do Teams licenças básicas podem ser usadas para licenciar Salas do Teams sistemas e não painéis do Teams.

Um sistema de Salas do Teams pode ser um dos seguintes:

- Um Salas do Teams para o módulo de computação do Windows e o console de toque anexado
- Um Salas do Teams para dispositivo Android e console de toque conectado por meio de uma conexão USB ou baseada em IP
- Um Surface Hub

As tarefas de administração remota executadas em um Salas do Teams ou dispositivo no centro de administração do Teams dependem da licença atribuída ao sistema ou dispositivo. Para obter mais informações, consulte [Salas do Microsoft Teams de licença no Centro de administração do Teams](admin-center-license-overview.md).

[!INCLUDE [mtr-user-licensing](../includes/mtr-user-licensing.md)]

## <a name="teams-rooms-license-service-plan-comparison"></a>Salas do Teams de plano de serviço de licença

A tabela a seguir mostra os serviços incluídos em cada Salas do Teams licença.

|                                           | Salas do Microsoft Teams Básico                                          | Salas do Microsoft Teams Pro                                              |
|:------------------------------------------|:--------------------------------------------------------------------:|:----------------------------------------------------------------------:|
| **Número máximo de licenças**            | 25                                                                   | Ilimitado                                                              |
| **Microsoft Teams**                       | &#x2714;                                                             | &#x2714;                                                               |
| **Audioconferência <sup>1</sup>**        | &#x2714;                                                             | &#x2714;                                                               |
| **Quadro de comunicações**                            | &#x2714;                                                             | &#x2714;                                                               |
| **Telefone do Teams**                           |                                                                      | &#x2714;                                                               |
| **Microsoft Intune**                      |                                                                      | &#x2714;                                                               |
| **Azure Active Directory Premium Plano 1** |                                                                      | &#x2714;                                                               |
| **Skype for Business Plano 2 <sup>2</sup>** |                                                                      | &#x2714;                                                               |
| **Disponibilidade geográfica**               | Mundial                                                            | Mundial                                                              |
| **Disponibilidade do segmento**                  | Comercial, Setor Público Mundial, Educação, Caridade, GCC         | Comercial, Setor Público Mundial, Educação, Caridade, GCC, GCC-High |
| **Disponibilidade de canal**                  | Web Direct, Nova experiência de comércio (NCE) – Cliente conduzido<sup>3</sup> | EA, EAS, EES, CSP, Web Direct, NCE – Cliente liderado, NCE – Parceiro liderado   |

<sup>1</sup> Para verificar a disponibilidade do serviço, consulte [Disponibilidade de país e região para Audioconferência e Planos de Chamadas](../country-and-region-availability-for-audio-conferencing-and-calling-plans/country-and-region-availability-for-audio-conferencing-and-calling-plans.md). [Os Créditos de](../what-are-communications-credits.md) Comunicação podem se aplicar a serviços adicionais, como chamada gratuita, minutos internacionais para planos domésticos e assim por diante. Você pode desabilitar esses recursos para evitar cobrança adicional.

<sup>2 Incluído</sup> para habilitar determinados métodos de autenticação herdados.

<sup>3</sup> Você precisa adicionar e atribuir uma licença Salas do Teams Basic gratuita para cada dispositivo por meio do Centro de administração do Microsoft 365.

## <a name="switching-from-teams-rooms-standard-and-teams-rooms-premium"></a>Alternando do Salas do Teams Standard e Salas do Teams Premium

Se sua organização tiver dispositivos de reunião que usam as licenças herdados Salas do Teams Standard ou Salas do Teams Premium, você poderá continuar usando essas licenças até a data de validade. Após a data de validade de suas licenças herdados existentes, recomendamos que você faça a transição para a licença Salas do Teams Pro. Os recursos de reunião e gerenciamento<sup>*</sup> disponíveis em ambas as licenças herdados foram combinados na licença Salas do Teams Pro.

> [!IMPORTANT]
> As licenças herdadas não fazem a transição automática para as novas licenças. Após a expiração de uma licença herdada, você precisará comprar uma nova licença Salas do Teams Pro (recomendada) ou Salas do Teams Basic. Salas do Teams pro podem ser adquiridas por meio do Centro de administração do Microsoft 365 ou do canal de vendas preferido. Salas do Teams licenças básicas só podem ser adquiridas por meio do Centro de administração do Microsoft 365.

Se sua organização tiver um Enterprise Agreement, você poderá continuar usando suas licenças herdados existentes até o próximo período de renovação. Você também pode continuar reservando licenças herdados adicionais até o próximo período de renovação. Para obter mais informações, entre em contato com seu representante da Microsoft.

Para obter informações sobre licenças herdados, [consulte Salas do Microsoft Teams licenças herdados](rooms-legacy-licensing.md).

<sup>*</sup>Os engenheiros de Serviço da Microsoft não servirão mais como intermediários para a resposta a incidentes a partir de 1º de outubro de 2022.

## <a name="teams-rooms-basic-and-teams-rooms-pro-feature-comparison"></a>Salas do Teams de recursos Básico e Salas do Teams Pro

As tabelas a seguir comparam as licenças Salas do Teams Basic e Salas do Teams Pro e mostram quais recursos estão disponíveis com cada um. Se um recurso não estiver disponível para uma licença, esse recurso não poderá ser usado em Salas do Teams que foram atribuídos a essa licença. Para usar um recurso em um Salas do Teams, atribua a licença na qual esse recurso está disponível para esse dispositivo.

### <a name="meeting-join"></a>Ingresso na reunião

|                                                                 | Salas do Microsoft Teams Básico | Salas do Microsoft Teams Pro |
|:----------------------------------------------------------------|:---------------------------:|:-------------------------:|
| **Ingressar em reuniões do Teams com 1 toque, proximidade e ID de reunião** | &#x2714;                    | &#x2714;                  |
| **Iniciar reuniões ad hoc da sala**                         | &#x2714;                    | &#x2714;                  |
| **Participação direta de convidados para reuniões do Zoom e do Webex**               | &#x2714;                    | &#x2714;                  |
| **Ingressar em reuniões entre nuvens do Teams**                           |                             | &#x2714;                  |
| **Check-in de sala com o Painel do Teams**                              |                             | &#x2714;                  |

### <a name="share-and-collaborate"></a>Compartilhar e colaborar

|                                                  | Salas do Microsoft Teams Básico | Salas do Microsoft Teams Pro |
|:-------------------------------------------------|:---------------------------:|:-------------------------:|
| **Compartilhar e exibir todos os tipos de conteúdo do Teams**       | &#x2714;                    | &#x2714;                  |
| **Compartilhar quadro de comunicações com a Câmera de Captura de Conteúdo** |                             | &#x2714;                  |

### <a name="meeting-engagement"></a>Compromisso da reunião

|                                                      | Salas do Microsoft Teams Básico | Salas do Microsoft Teams Pro |
|:-----------------------------------------------------|:---------------------------:|:-------------------------:|
| **Galeria de vídeos do Teams com várias opções de layout** | &#x2714;                    | &#x2714;                  |
| **Linha da frente**                                        |                             | &#x2714;                  |
| **Modo juntos**                                    |                             | &#x2714;                  |
| **Galeria grande (até 50 vídeos)**                  |                             | &#x2714;                  |
| **Suporte a tela dupla**                              |                             | &#x2714;                  |

### <a name="calling"></a>Chamadas

|                                                   | Salas do Microsoft Teams Básico | Salas do Microsoft Teams Pro |
|:--------------------------------------------------|:---------------------------:|:-------------------------:|
| **Fazer e receber chamadas ponto a ponto e de grupo** | &#x2714;                    | &#x2714;                  |
| **Sistema de Telefonia do Microsoft 365**                    |                             | &#x2714;                  |

### <a name="intelligent-audio-and-video"></a>Áudio e vídeo inteligentes

|                                                                                 | Salas do Microsoft Teams Básico | Salas do Microsoft Teams Pro |
|:--------------------------------------------------------------------------------|:---------------------------:|:-------------------------:|
| **Suporte para transcrição ao vivo do Locutor Inteligente com identificação do locutor** |                             | &#x2714;                  |
| **Suporte a várias câmeras**                                                        |                             | &#x2714;                  |
| **Modo de exibição de sala panorâmico**                                                         |                             | &#x2714;                  |
| **Supressão de ruído de IA**                                                        |                             | &#x2714;                  |
| **Pessoas contagem**                                                             |                             | &#x2714;                  |

### <a name="security-and-compliance"></a>Segurança e conformidade

|                                                                     | Salas do Microsoft Teams Básico | Salas do Microsoft Teams Pro |
|:--------------------------------------------------------------------|:---------------------------:|:-------------------------:|
| **Sistema operacional seguro**                                         | &#x2714;                    | &#x2714;                  |
| **Segurança no nível do sistema (inicialização segura, modo de acesso atribuído etc.)** | &#x2714;                    | &#x2714;                  |
| **Azure AD políticas de acesso de condição**                              |                             | &#x2714;                  |

### <a name="device-management"></a>Gerenciamento de dispositivos

|                                                   | Salas do Microsoft Teams Básico | Salas do Microsoft Teams Pro |
|:--------------------------------------------------|:---------------------------:|:-------------------------:|
| **Registro e inventário Administração Central de Equipes**   | &#x2714;                    | &#x2714;                  |
| **Atualizações automáticas de software**                    | &#x2714;                    | &#x2714;                  |
| **Informações detalhadas sobre o sistema e a configuração** |                             | &#x2714;                  |
| **Gerenciamento de integridade periférico**                  |                             | &#x2714;                  |
| **Configuração remota**                          |                             | &#x2714;                  |
| **Histórico e atividade do dispositivo**                   |                             | &#x2714;                  |
| **Integração de ITSM**                              |                             | &#x2714;                  |
| **Alertas de integridade personalizados**                          |                             | &#x2714;                  |
| **Análise de dispositivo**                              |                             | &#x2714;                  |
