...
ms.collection:
- M365-collaboration
description: Saiba como proteger seus dispositivos Salas do Microsoft Teams de segurança.
...
ms.locfileid: "63368376"
---
# <a name="microsoft-teams-rooms-security"></a>Salas do Microsoft Teams Segurança

A Microsoft trabalha com nossos parceiros para fornecer uma solução segura e que não exige ações adicionais para proteger Salas do Microsoft Teams. Este artigo discute muitos dos recursos de segurança encontrados no Salas do Teams.
...
> Este artigo se aplica a Salas do Microsoft Teams em execução em Windows.
...
Proteção de DMA (Acesso direto à memória) do kernel é uma configuração Windows 10 que está habilitada em Salas do Teams. Com esse recurso, o sistema operacional e o firmware do sistema protegem o sistema contra ataques DMA mal-intencionados e não intencionais para todos os dispositivos compatíveis com DMA:
...
Depois que o Microsoft Windows inicializar, Salas do Teams entrar automaticamente em uma conta de usuário local Windows chamada Skype. A Skype conta não tem senha. Para tornar a sessão Skype conta segura, as etapas a seguir são tomadas.
...
Você precisa criar uma conta de recurso para cada dispositivo Salas do Teams para que ele possa entrar Teams. Não é possível usar a autenticação de dois fatores ou multifações com essa conta. Exigir um segundo fator impediria que a conta fosse capaz de entrar automaticamente no aplicativo Salas do Teams após uma reinicialização. No entanto, você pode habilitar a Autenticação Moderna para obter segurança adicional para essa conta. Além disso, Azure Active Directory políticas de Acesso Condicional e Políticas de Conformidade do Intune podem ser implantadas para proteger a conta de recurso. Para obter mais informações, consulte [Supported Conditional Access and Intune device compliance policies for Salas do Microsoft Teams](supported-ca-and-compliance-policies.md) and [Conditional Access and Intune compliance for Salas do Microsoft Teams](conditional-access-and-compliance-for-devices.md)

Recomendamos que você crie a conta de recurso no Azure AD, se possível. Embora uma conta sincronizada possa funcionar com Salas do Teams em implantações híbridas, essas contas sincronizadas geralmente têm dificuldade de entrar no Salas do Teams e podem ser difíceis de solucionar problemas. Se você optar por usar um serviço de federação de terceiros para autenticar as credenciais da conta de recurso, verifique se o IDP `wsTrustResponse` de terceiros responde com o atributo definido como `urn:oasis:names:tc:SAML:1.0:assertion`.

## <a name="network-security"></a>Segurança de Rede

Geralmente, Salas do Teams tem os mesmos requisitos de rede que qualquer cliente Microsoft Teams cliente. O acesso por firewalls e outros dispositivos de segurança é o mesmo Salas do Teams para qualquer outro cliente Microsoft Teams. Específicas Salas do Teams, as categorias listadas como "necessárias" para Teams devem estar abertas no firewall. Salas do Teams também precisa de acesso Windows Update, Microsoft Store e Microsoft Intune (se você usar Microsoft Intune gerenciar seus dispositivos). Para ver a lista completa de IPs e URLs necessárias para Salas do Microsoft Teams, consulte:
