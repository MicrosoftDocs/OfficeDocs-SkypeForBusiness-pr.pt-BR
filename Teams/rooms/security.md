...
- **As credenciais** derivadas Windows de NTLM e Kerberos baseadas em virtualização e outros segredos são executadas em um ambiente protegido isolado do sistema operacional em execução.
...
Depois que o Microsoft Windows inicializado, Salas do Teams automaticamente entra em uma conta de usuário local Windows chamada Skype. A Skype conta não tem senha. Para tornar a sessão Skype conta segura, as etapas a seguir são executadas.

> [!IMPORTANT]
> Não altere a senha nem edite a conta de Skype usuário local. Isso pode impedir que Salas do Teams entre automaticamente.

O Salas do Microsoft Teams aplicativo é executado usando o recurso Acesso Atribuído encontrado no Windows 10 1903 e posterior. O Acesso Atribuído é um recurso no Windows 10 que limita os pontos de entrada do aplicativo expostos ao usuário. Isso é o que habilita o modo de quiosque de aplicativo único. Usando o Iniciador de Shell, Salas do Teams é configurado como um dispositivo de quiosque que executa um Windows da área de trabalho como a interface do usuário. O Salas do Microsoft Teams substitui o shell padrão (explorer.exe) que geralmente é executado quando um usuário faz logon. Em outras palavras, o shell tradicional do Explorer não é iniciado. Isso reduz consideravelmente a superfície Salas do Microsoft Teams vulnerabilidade dentro Windows. Para obter mais informações, consulte [Configurar quiosques e sinais digitais Windows edições da área de trabalho](/windows/configuration/kiosk-methods).

Se você decidir executar uma verificação de segurança ou um parâmetro de comparação do CIS (Center for Internet Security) no Salas do Teams, a verificação só poderá ser executada no contexto de uma conta de administrador local, pois a conta de usuário do Skype não dá suporte à execução de aplicativos diferentes do aplicativo Salas do Teams. Muitos dos recursos de segurança aplicados ao contexto de usuário do Skype não se aplicam a outros usuários locais e, como resultado, essas verificações de segurança não exibirão o bloqueio de segurança completo aplicado à conta Skype segurança. Portanto, não é recomendável executar uma verificação local no Salas do Teams. No entanto, você pode executar testes de penetração externos, se assim desejar. Por isso, recomendamos que você execute testes de penetração externos em Salas do Teams em vez de executar verificações locais.

Além disso, as políticas de bloqueio são aplicadas para limitar o uso de recursos não administrativos. Um filtro de teclado está habilitado para interceptar e bloquear combinações de teclado potencialmente inseguras que não são cobertas pelas políticas de Acesso Atribuído. Somente usuários com direitos administrativos locais ou de domínio têm permissão para entrar Windows gerenciar Salas do Teams. Essas e outras políticas aplicadas ao Windows em Salas do Microsoft Teams dispositivos são continuamente avaliadas e testadas durante o ciclo de vida do produto.
...
Você também pode importar contas de domínio para o grupo Windows Administrador local. Você pode fazer isso para Azure AD contas usando Intune. Para obter mais informações, consulte [CSP de política – RestrictedGroups.](/windows/client-management/mdm/policy-csp-restrictedgroups).
...
Windows Designer de Configuração pode ser usado para criar Windows 10 de provisionamento. Além de alterar a senha Administração local, você também pode fazer coisas como alterar o nome do computador e registrar-se no Azure Active Directory. Para obter mais informações sobre como criar um Windows de provisionamento do Designer de Configuração, consulte [Pacotes de provisionamento para Windows 10](/windows/configuration/provisioning-packages/provisioning-packages).

Você precisa criar uma conta de recurso para cada Salas do Teams para que ele possa entrar Teams. Você não pode usar a autenticação multifator ou de dois fatores com essa conta. Exigir um segundo fator impediria que a conta fosse capaz de entrar automaticamente no aplicativo Salas do Teams após uma reinicialização. No entanto, você pode habilitar a Autenticação Moderna para segurança adicional para essa conta. Além disso, Azure Active Directory políticas de acesso condicional e Intune políticas de conformidade podem ser implantadas para proteger a conta de recurso. Para obter mais informações, consulte Acesso Condicional e políticas de conformidade do dispositivo [Intune](supported-ca-and-compliance-policies.md) com suporte para Salas do Microsoft Teams e acesso condicional e conformidade [Intune para](conditional-access-and-compliance-for-devices.md) Salas do Microsoft Teams
...
Se você estiver usando o Salas do Microsoft Teams de serviços gerenciados do Salas do Microsoft Teams Premium, também precisará verificar se o Salas do Teams pode acessar as seguintes URLs:
...
Salas do Teams é configurado para se manter automaticamente corrigido com as atualizações de Windows mais recentes, incluindo atualizações de segurança. Salas do Teams instala todas as atualizações pendentes todos os dias a partir das 2h usando uma política local predefinido. Não é necessário usar ferramentas adicionais para implantar e aplicar Windows Atualizações. O uso de ferramentas adicionais para implantar e aplicar atualizações pode atrasar a instalação Windows patches e, portanto, levar a uma implantação menos segura. O Salas do Teams aplicativo é implantado usando o Microsoft Store. Se os dispositivos forem licenciados com Salas do Microsoft Teams Padrão, todas as novas versões do aplicativo serão instaladas automaticamente durante o processo de aplicação de patch noturna. Se os dispositivos forem licenciados com Salas do Microsoft Teams Premium e registrados no Serviço Gerenciado da Microsoft, novas versões do aplicativo Salas do Teams serão instaladas de acordo com o plano de distribuição definido.
...
Para obter um desempenho ideal de mídia em tempo real, é altamente recomendável configurar Teams de mídia para ignorar servidores proxy e outros dispositivos de segurança de rede. A mídia em tempo real é sensível à latência, e os servidores proxy e os dispositivos de segurança de rede podem degradar significativamente a qualidade de áudio e vídeo dos usuários. Além disso, como Teams mídia já está criptografada, não há nenhum benefício tangível de passar o tráfego por meio de um servidor proxy. Para obter mais informações, consulte Rede até (para a nuvem[) —](/microsoft-365/solutions/networking-design-principles?view=o365-worldwide) Ponto de vista de um arquiteto que discute as recomendações de rede para melhorar o desempenho da mídia com Microsoft Teams e Salas do Microsoft Teams.
...
O Ingresso por Proximidade e outros Salas do Teams recursos dependem de Bluetooth. No entanto, Bluetooth implementação em Salas do Teams dispositivos não permite uma conexão de dispositivo externo com um Salas do Teams dispositivo. Bluetooth uso de tecnologia em Salas do Teams dispositivos está limitado atualmente a sinalizadores de publicidade e conexões de conexões de código. O `ADV_NONCONN_INT` tipo de PDU (unidade de dados de protocolo) é usado no sinalizador de publicidade. Esse tipo de PDU é para dispositivos não conectáveis que anunciam informações para o dispositivo de escuta. Não há nenhum Bluetooth de dispositivos como parte desses recursos. Detalhes adicionais sobre Bluetooth protocolos podem ser encontrados [no site Bluetooth SIG](https://www.bluetooth.com/blog/bluetooth-low-energy-it-starts-with-advertising/).
