...
> [!NOTE]
> Extrair os arquivos do MSI pode ser realizado por vários meios. Qualquer mecanismo que extraia todos os arquivos e preserva sua estrutura de diretórios é aceitável. Uma dessas maneira `msiexec /a PathToMsi /qb TARGETDIR=PathToTarget` `PathToMsi` é usar o comando onde representa o caminho completo para o pacote de instalação da sala Microsoft Teams e representa o caminho completo para a `PathToTarget` pasta à qual você gostaria que os arquivos extraídos.
...
2) Verifique no dispositivo Salas do Microsoft Teams `RecoveryTool.ps1` que você pode acessar o arquivo, que está incluído nos arquivos extraídos do pacote de instalação Salas do Microsoft Teams. O kit pode ser encontrado no compartilhamento de rede ou unidade USB usado na preparação de pré-requisitos.
...
4) Para executar uma restauração de fábrica:
   ...
   3. Execute a restauração de fábrica.
      ...
      3. Abaixo **de Redefinir este computador**, selecione **Começar**
      4. Selecione **Remover tudo** e, em **seguida, Next** e **Reset**
        > [!WARNING]
        > O Salas do Microsoft Teams dispositivo pode se tornar inutilizável se a opção Manter meus arquivos **- Remove Aplicativos** e configurações, mas mantém sua opção de arquivos pessoais selecionada durante o processo Windows Redefinição. Não selecione essa opção.
