---
title: Exportar conteúdo com as APIs Microsoft Teams Exportação
author: SerdarSoysal
ms.author: serdars
manager: serdars
ms.topic: reference
...
description: Neste artigo, você aprenderá a exportar Teams conteúdo usando as APIs Microsoft Teams Exportação.
...
---
# <a name="export-content-with-the-microsoft-teams-export-apis"></a>Exportar conteúdo com as APIs Microsoft Teams Exportação

Teams APIs de Exportação permitem exportar 1:1, chat em grupo, chats de reunião e mensagens de canal de Microsoft Teams. Se sua organização precisar exportar Microsoft Teams mensagens, você poderá extraí-las usando apIs Teams Exportação. *A Mensagem de Chat* representa uma mensagem de chat individual dentro [de um canal ou](/graph/api/resources/channel) [chat](/graph/api/resources/chat). A mensagem de chat pode ser uma mensagem de chat raiz ou parte de um thread de resposta definido pela propriedade **replyToId** na mensagem de chat.

Aqui estão alguns exemplos de como você pode usar essas APIs de exportação:

- **Exemplo 1**: se você habilitou o Microsoft Teams em sua organização e deseja exportar todas as mensagens Microsoft Teams para datar programaticamente passando o intervalo de datas para um determinado usuário ou equipe.
...
## <a name="what-is-supported-by-the-teams-export-apis"></a>O que é compatível com as APIs Teams Exportação?

- Exportação em massa de **Teams Mensagem:** apIs de exportação do Teams dão suporte a até 200 RPS por aplicativo por locatário e 600 RPS para um aplicativo, com esses limites, você deve ser capaz de exportar em massa mensagens Teams aplicativo.
...
- **Ambiente Híbrido:** As APIs de exportação dão suporte a mensagens enviadas por usuários provisionados no Ambiente Híbrido (ambiente Exchange e Teams). Todas as mensagens enviadas por usuários configurados para o ambiente híbrido estarão acessíveis usando APIs de Exportação.
...
- **Propriedades da mensagem de chat:** Consulte a lista completa de propriedades que dão suporte Teams EXPORTAR APIs [aqui](/graph/api/resources/chatmessage#properties).
...
## <a name="prerequisites-to-access-teams-export-apis"></a>Pré-requisitos para acessar as APIs Teams Exportação

- Microsoft Teams APIs no Microsoft Graph que acessam dados confidenciais são consideradas APIs protegidas. As APIs de exportação exigem que você tenha validação adicional, além de permissões e consentimento, antes de poder usá-las. Para solicitar acesso a essas APIs protegidas, preencha o formulário [de solicitação](https://aka.ms/teamsgraph/requestaccess).
...
## <a name="license-requirements-for-teams-export-apis"></a>Requisitos de licença para apIs Teams Exportação
