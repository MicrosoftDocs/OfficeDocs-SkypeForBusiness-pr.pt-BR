...
Uma fila de chamadas é análoga a uma sala de espera em um prédio físico. Os chamadores aguardam em espera enquanto as chamadas são roteadas para os agentes na fila à medida que se tornam disponíveis. As filas de chamadas são geralmente usadas para vendas e funções de serviço, mas podem ser usadas para qualquer situação em que o número de chamadas excede a sua capacidade interna, como uma recepcionista em uma facilidade de ocupação.
...
Para configurar atendedores automáticos e filas de chamadas, você precisa do seguinte:
...
Documente as respostas a essas perguntas e forneça as informações de que o administrador está fazendo a configuração.
...
- Para os prompts de voz, você quer gravar seu próprio ou usar a voz gerada pelo sistema? (A voz gerada pelo sistema é muito fácil de atualizar.)
...
Contas de equipe dos agentes devem ser definidas no modo somente para equipes. Os agentes que não atendem aos requisitos não são incluídos na lista de circulação de chamadas.
...
Como parte do processo de planejamento, recomendamos que você trabalhe no roteamento de chamadas para sua organização em um diagrama. Isso ajudará a determinar o roteamento mais eficiente para as pessoas que fizerem chamadas para a sua organização. Você também pode usar o diagrama para determinar os atendedores automáticos e as filas de chamadas que você precisa criar, juntamente com requisitos relacionados, como números de serviço, licenças e contas de recursos.
...
- **Tempo limite da chamada** -se um chamador estiver na fila mais que a configuração de tempo limite configurada, ele será redirecionado para fora da fila.
...
- Se você receber muitas chamadas solicitando informações básicas sobre a sua empresa, como horário comercial, local ou endereço do site, considere a criação de um atendedor automático para responder a essas perguntas com mensagens gravadas.
