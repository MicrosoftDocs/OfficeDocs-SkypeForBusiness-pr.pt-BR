...
![Os componentes principais dos eventos ao vivo.](../media/live-events-flow-diagram.png  "Principais componentes de eventos ao vivo, agendamento, produção, provedores eCDN certificados de terceiros")
...
- **Codificador do Teams**: permite que os usuários produzam seus eventos ao vivo diretamente de um codificador externo baseado em hardware ou software com [o Teams](../teams-stream-overview.md). Essa é a melhor opção se você já tiver equipamento com qualidade de estúdio (por exemplo, mixers de mídia) com suporte de streaming para um serviço de Protocolo de Mensagens em Tempo Real (RMTP). Esse tipo de produção geralmente é usado em eventos em grande escala como um evento corporativo - em que um único stream de um mixer de mídia é transmitido para a audiência. 
...
- [Eventos de streaming ao vivo no Microsoft Teams](../teams-stream-overview.md)
