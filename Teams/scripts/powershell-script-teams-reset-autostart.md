...
ms.service: msteams
audience: admin
description: Use este script do PowerShell para redefinir a configuração de início automático Teams por usuário.
f1.keywords:
- NOCSH
...
ms.locfileid: "51117259"
---
# <a name="powershell-script-sample---reset-the-autostart-setting-in-teams"></a><span data-ttu-id="a171b-103">Exemplo de script do PowerShell - Redefinir a configuração de início automático no Teams</span><span class="sxs-lookup"><span data-stu-id="a171b-103">PowerShell script sample - Reset the autostart setting in Teams</span></span>

<span data-ttu-id="a171b-104">Use este script para redefinir Teams configuração de início automático por usuário.</span><span class="sxs-lookup"><span data-stu-id="a171b-104">Use this script to reset the Teams autostart setting on a per-user basis.</span></span> <span data-ttu-id="a171b-105">Isso inclui todos os valores definidos pelo usuário ou pelo Teams app.</span><span class="sxs-lookup"><span data-stu-id="a171b-105">This includes any values set by the user or the Teams app.</span></span> <span data-ttu-id="a171b-106">Por padrão, Teams inicia automaticamente quando um usuário faz logona no computador depois de instalado.</span><span class="sxs-lookup"><span data-stu-id="a171b-106">By default, Teams automatically starts when a user logs in to their computer after it's installed.</span></span>

<span data-ttu-id="a171b-107">Se você já tiver implantado o Teams e quiser definir a configuração Impedir que o [Microsoft Teams](../msi-deployment.md#use-group-policy-recommended) seja iniciado automaticamente após a instalação da Política de Grupo para desabilitar Teams início automático, você precisará primeiro definir a configuração de Política de Grupo como o valor que deseja e executar esse script.</span><span class="sxs-lookup"><span data-stu-id="a171b-107">If you've already deployed Teams and want to set the [Prevent Microsoft Teams from starting automatically after installation Group Policy setting](../msi-deployment.md#use-group-policy-recommended) to disable Teams autostart, you'll need to first set the Group Policy setting to the value you want, and then run this script.</span></span>

<span data-ttu-id="a171b-108">Depois Teams for iniciado para um usuário, as configurações de início automático não poderão ser desabilitadas usando a Política de Grupo.</span><span class="sxs-lookup"><span data-stu-id="a171b-108">After Teams is started for a user, the autostart settings can't be disabled by using Group Policy.</span></span>

## <a name="sample-script"></a><span data-ttu-id="a171b-109">Script de exemplo</span><span class="sxs-lookup"><span data-stu-id="a171b-109">Sample script</span></span>
...
## <a name="related-topics"></a><span data-ttu-id="a171b-110">Tópicos relacionados</span><span class="sxs-lookup"><span data-stu-id="a171b-110">Related topics</span></span>

- [<span data-ttu-id="a171b-111">Instalar o Teams usando MSI</span><span class="sxs-lookup"><span data-stu-id="a171b-111">Install Teams using MSI</span></span>](../msi-deployment.md)
- [<span data-ttu-id="a171b-112">Implantar Teams com Microsoft 365 Apps para Grandes Empresas</span><span class="sxs-lookup"><span data-stu-id="a171b-112">Deploy Teams with Microsoft 365 Apps for enterprise</span></span>](/deployoffice/teams-install)