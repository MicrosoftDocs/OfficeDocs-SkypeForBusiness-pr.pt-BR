...
Para opções de discagem, atribua as chaves 0-9 no teclado telefônico a um dos destinos de roteamento de chamadas. As chaves \* (asterisco) e \# (libra) são reservadas pelo sistema e não podem ser reatribuídas. Pressionar uma dessas chaves repetirá o menu atual.

> [!NOTE]
> A chave # só faz backup do atendente automático mais recente. Depois que o limite for cruzado para um novo atendente automático, a chave # não poderá levá-lo ao anterior.
...
*Nova opção – A opção Forçar escuta pode ser habilitada, o que exige que os chamadores ouçam todas as opções de menu antes de fazer a seleção.*
 *Novas chaves - \* (asterisco) e \# (libra) agora podem ser usadas nas opções de menu.*
 *Novas **opções de menu – Reprodução** agora estão disponíveis nos fluxos de chamadas de feriado.*
