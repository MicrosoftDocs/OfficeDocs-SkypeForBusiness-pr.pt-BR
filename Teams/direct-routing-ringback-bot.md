...
Este artigo descreve o bot ringback, que você pode usar para ajudar a evitar silêncios inesperados que podem ocorrer quando leva mais tempo para que as chamadas sejam estabelecidas. O bot ringback está disponível para Roteamento Direto no modo de bypass não mídia.

Às vezes, as chamadas de entrada da PSTN (Rede Telefônica Pública Comucionária) para clientes do Teams podem demorar mais do que o esperado. Isso pode ocorrer por vários motivos. Quando isso acontece, o chamador pode não ouvir nada, o cliente do Teams não toca e alguns provedores de telecomunicações podem cancelar a chamada.

O bot ringback ajuda a evitar silêncios inesperados que podem ocorrer nesse cenário. Para chamadas de entrada do PSTN para clientes do Teams, o bot ringback reproduz um sinal de áudio distinto para o chamador para indicar que o Teams está em processo de estabelecer a chamada.

> [!NOTE]
> O bot ringback gera mídia inicial do back-back do Teams. Em alguns países e regiões, você pode ser cobrado pela chamada quando a mídia começa a fluir.
