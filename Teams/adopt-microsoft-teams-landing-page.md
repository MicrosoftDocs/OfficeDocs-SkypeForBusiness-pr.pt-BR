---
title: Adotar o Microsoft Teams
ms.reviewer: karuanag
description: Saiba mais sobre os recursos de adoção disponíveis para ajudá-lo a planejar e implantar Microsoft Teams em sua organização.
ms.topic: overview
author: serdarsoysal
ms.author: serdars
manager: serdars
audience: admin
ms.service: msteams
ms.collection:
- Teams_ITAdmin_Adopt
- M365-collaboration
f1.keywords:
- CSH
ms.custom:
- ms.teamsadmincenter.dashboard.helparticle.adoption
- Adopt
search.appverid: MET150
appliesto:
- Microsoft Teams
ms.openlocfilehash: be0e637d1242cbb60ca95d9fe05113aba6878b97
ms.sourcegitcommit: 15e90083c47eb5bcb03ca80c2e83feffe67646f2
ms.translationtype: MT
ms.contentlocale: pt-BR
ms.lasthandoff: 08/30/2021
ms.locfileid: "58725650"
---
# <a name="adopt-microsoft-teams"></a>Adotar o Microsoft Teams

> [!NOTE]
> Para obter uma visão geral de como fazer a transição para o aprendizado remoto e recursos para ajudá-lo a começar, consulte Começar a Microsoft Teams [para aprendizagem remota.](remote-learning-edu.md)

Se você quiser lançar um Teams começar com chat, equipes & canais e reuniões, use nossas diretrizes prescritivas De início [,](get-started-with-teams-quick-start.md)que foi projetada para que você seja iniciado e executado rapidamente. Se você for uma grande organização com uma configuração híbrida ou local Skype for Business ou se quiser lançar recursos de voz (como planos de chamadas ou sistemas telefônicos do Microsoft 365 ou do Office 365), você pode começar com nossas diretrizes De início, mas precisará das orientações adicionais abaixo, sob orientação de adoção mais [profunda.](#deeper-adoption-guidance)

|               |               |               |               |               |               |
| :-------------| :-------------| :-------------| :-------------| :-------------| :-------------|
| <img src="/office/media/icons/clock-teams.svg" alt="clock" width="40 px" height="40 px"> | [Início rápido](./teams-adoption-quick-start-checklist.md) | <img src="/office/media/icons/chat.svg" alt="chat bubbles" width="40 px" height="40 px"> | [Introdução às equipes e canais](./teams-adoption-understand-teams-and-channels.md) | <img src="/office/media/icons/task-checklist-planning-teams.svg" alt="checklist" width="40 px" height="40 px"> | [Planejar suas primeiras equipes](./teams-adoption-your-first-teams.md) |
| <img src="/office/media/icons/best-practices-teams.svg" alt="best practices" width="40 px" height="40 px"> | [Capacitar seus campeões](./teams-adoption-create-champions-program.md) | <img src="/office/media/icons/education-tutorial-teams.svg" alt="prize ribbon" width="40 px" height="40 px"> | [Treinamento](./training-microsoft-teams-landing-page.md) | <img src="/office/media/icons/toolbox.svg" alt="wrench" width="40 px" height="40 px"> | [Ferramentas e Downloads](./adopt-tools-and-downloads.md) |

### <a name="deeper-adoption-guidance"></a>[Diretrizes de adoção mais profundas](#deeper-adoption-guidance)

Os planos de adoção podem ser simples de complexos, dependendo do ambiente. Para implantações em grande escala, confira as diretrizes abaixo para garantir que sua organização tenha uma transição suave para Teams.

|               |               |               |               |               |               |
| :-------------| :-------------| :-------------| :-------------| :-------------| :-------------|
| <img src="/office/media/icons/circle-number-1-teams.svg" alt="circle number one" width="40 px" height="40 px"> | **[Start](./teams-adoption-phase1.md)** <br/> [Introdução](./teams-adoption-get-started.md) <br/><br/> [Entender as equipes e canais](./teams-adoption-understand-teams-and-channels.md) <br/><br/> [Criar sua primeira equipe](./teams-adoption-your-first-teams.md) <br/><br/> [Avaliar a preparação cultural e organizacional](./teams-adoption-assess-readiness.md) | <img src="/office/media/icons/circle-number-2-teams.svg" alt="circle number 2" width="40 px" height="40 px"> | **[Experimento](./teams-adoption-phase2-experiment.md)** <br/> [Criar seu programa de defensores](./teams-adoption-create-champions-program.md) <br/><br/> [Início rápido de governança](./teams-adoption-governance-quick-start.md)<br/><br/> [Definir os cenários de uso](./teams-adoption-define-usage-scenarios.md) <br/><br/> [Integração de usuários pioneiros e coleta de comentários](./teams-adoption-onboard-early-adopters.md) <br/><br/> [Suporte à integração](./teams-adoption-onboard-support.md) | <img src="/office/media/icons/circle-number-3-teams.svg" alt="circle number 3" width="40 px" height="40 px"> | **[Escala](./teams-adoption-phase3-enable.md)** <br/> [Definir resultado e sucesso](./teams-adoption-define-outcomes.md) <br/><br/> [Otimizar comentários e relatórios](./teams-adoption-optimize-feedback-and-reporting.md) <br/><br/> [Promover a conscientização e implementar treinamento](./teams-adoption-drive-awareness.md) <br/><br/> [Agendar avaliações de integridade do serviço](./teams-adoption-schedule-service-health-reviews.md) |
